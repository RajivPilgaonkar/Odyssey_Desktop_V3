...
import { getDevExtremeTable, tableHeaderArray } from "./GetVoucherRepeatPymtData";
...
function VoucherRepeatPymt(props) {
  ...
  const quickCostEntry = async () => {
    ...
    const query = "SELECT vp.VoucherPayments_id, v.Vouchers_id, " + 
      "v.VoucherNo, v.Description, vp.CashVoucherNo, vp.Details, v.ExpectedCost, vp.Db " +
      "FROM Vouchers v " +
      "INNER JOIN VoucherPayments vp ON v.Vouchers_id = vp.Vouchers_id " +
    ...
    compVar.quickEntryHeaderData = [
      {field: 'VoucherPayments_id', caption: 'ID', allowEditing: false, width: 60, visible: false, dataType: 'number'},
      ...
      {field: 'CashVoucherNo', caption: 'CashVoucher No', allowEditing: false, width: 120, visible: true, dataType: 'number'},
      {field: 'Details', caption: 'Details', allowEditing: false, width: 200, visible: true, dataType: 'string'},
      {field: 'ExpectedCost', caption: 'Exp. Cost', allowEditing: false, width: 100, visible: true, dataType: 'number'},
      {field: 'Db', caption: 'Amount', allowEditing: true, width: 100, visible: true, dataType: 'number'},
    ];
  ...
  const onQuickClose = async () => {
    ...
    await updatePymtSummary();
    await filterData();
    forceRender();
  }

  //**********************************************************/
  const updatePymtSummary = async () => {

    for (const rec of compVar.quickEntryData) {

      const sql = 'UPDATE Vouchers SET AmountPaid = (' + 
        'SELECT SUM(COALESCE(Db,0.0)) FROM VoucherPayments vp WHERE vp.vouchers_id = Vouchers.vouchers_id) ' +
  ...
  const updateBills = async () => {
    ...
    for (const rec of compVar.mainData.filter(rec => rec.Selected)) {
      const sql = 'EXEC [p_UpdateVoucherPymts] ' + 
  ...
  const createElementProps = () => {
    ...
    return {
      numButtons: 1,
      buttonListObj: [
        {visible: true, options: {icon: "icons/updateDetails.png", onClick: updateBills, hint: 'Update Pymt Details for these vouchers'}},
        {visible: true, options: {icon: "icons/quickEntry.png", onClick: quickCostEntry, hint: 'Quick Pymt Entries'}},
      ],
      ...
      <>
        ...
        {compVar.displayQuickCost &&
          <CostQuickEntry
            ...
            tableName={'VoucherPayments'}
            keyField={'VoucherPayments_id'}
...
export default VoucherRepeatPymt;
