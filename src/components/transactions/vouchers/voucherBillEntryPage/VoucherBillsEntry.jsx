...
import { beforeInsert, saveEditedInsertedData, checkNullErrors, getFieldsArray } from "../../../common/CommonTransactionFunctions";
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetVoucherBillsEntryData";
...
import {getBilledAmount} from "../../../common/VoucherHelpers";
import PopupDialogBox from '../../../common/PopupDialogBox';
import VoucherRepeatBill from "./voucherRepeatBillPage/VoucherRepeatBill";

import '../../../common/MasterGrid.css'

let compVar = {};

function VoucherBillsEntry(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'VoucherBillings', keyField: 'VoucherBillings_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: '',
      mainTitle: 'Voucher Billing', title: 'New Voucher Billing',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['BillDate, BillNo'], table: 'VoucherBillings', where: whereStr, x_uid: _g_users_id, x_module: 'Voucher Bills'});   
    } catch(err) {
      alert(err);
    }
  ...
  const saveFormData = async () => {
    ...
    const amountBilled = await getBilledAmount(props.vouchers_id);
    await updateAmountBilled(props.vouchers_id, amountBilled);
  ...
  //**********************************************************/
  const updateAmountBilled = async (vouchers_id, amt) => {

    let sql = "UPDATE Vouchers SET AmountBilled = " + amt.toString() + " " +
  ...
  //**********************************************************/
  const onBillsSwitchChange = (e) => {
    compVar.displayRepeatEntry = !compVar.displayRepeatEntry;
    forceRender();
  }
      ...
      <>
        <div className="master-grid-container" style={{height: containerHeight, background: '#ffefcc'}}>
          ...
          <div style={{display: 'flex', flexDirection: 'column'}}>
            ...
            <div style={{display: 'flex', width: '100%', justifyContent: 'center', alignItems: 'center', fontSize: 18}}>
              { `Voucher No: ${props.activeVoucher.VoucherNo}, ${props.activeVoucher.Organisation}` }
            </div>

            <div style={{display: 'flex', width: '100%', justifyContent: 'center', alignItems: 'center', fontSize: 18, paddingTop: 10}}>
              Expected Cost: {props.activeVoucher.ExpectedCost} 
            </div>

            <div style={{display: 'flex', width: '100%', justifyContent: 'center', alignItems: 'center', fontSize: 16, paddingTop: 10, paddingBottom: 20}}>
              {props.activeVoucher.Description} 
            </div>

            {compVar.mainData.length > 0 &&
              <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'center', paddingBottom: 20}}>
                <div className="master-grid-params-switch-label" style={{paddingLeft: 20, fontSize: 18, display: 'flex', alignItems: 'center'}}>
                  Update Other Bills
                </div>
                <div style={{height: 20, display: 'flex', alignItems: 'center' }}>
                  <Switch height={20} width={40} onChange={onBillsSwitchChange} checked={compVar.displayRepeatEntry} uncheckedIcon={false} />
                </div>
              </div>
            }
...
export default VoucherBillsEntry;
