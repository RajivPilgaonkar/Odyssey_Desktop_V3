...
//**********************************************************/
export async function exportCarCityGroupBreakup(reportObj, priceData, worksheet) {

  let cell = worksheet.getCell('A1');
  cell.value = 'Car City Groups';  
 ...
 // cell.font = { name: 'Calibri', size: 12, bold: true };
  ...
  const columns = [
    ...
    {id: 'parkcost', col: 'M', title: 'Parking', width: 9, numformat: '#,##0', alignment: 'right', field: 'ParkCost', printIfZero: false},
    {id: 'nighthalt', col: 'N', title: 'Night Halt', width: 9, numformat: '#,##0', alignment: 'right', field: 'NightHaltCost', printIfZero: false},
    {id: 'tolltax', col: 'O', title: 'Toll Tax', width: 9, numformat: '#,##0', alignment: 'right', field: 'TollTax', printIfZero: false},
    {id: 'escort', col: 'P', title: 'Escort', width: 9, numformat: '#,##0', alignment: 'right', field: 'EscortCost', printIfZero: false},
    {id: 'comm', col: 'Q', title: 'Commission', width: 9, numformat: '#,##0', alignment: 'right', field: 'Commission', printIfZero: false},
    {id: 'vendorgst', col: 'R', title: 'GST', width: 9, numformat: '#,##0', alignment: 'right', field: 'VendorGst', printIfZero: false},
    {id: 'total', col: 'S', title: 'Total', width: 9, numformat: '#,##0', alignment: 'right', field: 'TotalCost', printIfZero: false},
    {id: 'margin', col: 'T', title: 'Margin', subtitle: marginStr, width: 9, numformat: '#,##0', alignment: 'right', field: 'Margin', field2: '', printIfZero: false},
    {id: 'quote', col: 'U', title: 'Quote', width: 9, numformat: '#,##0', alignment: 'right', field: 'Quote', field2: '', printIfZero: false},
    {id: 'b5', col: 'V', title: '', width: 3, alignment: '', field: ''},
    {id: 'quotegst', col: 'W', title: 'Quote incl. GST', subtitle: 'INR', width: 9, numformat: '#,##0', alignment: 'right', field: 'FinalQuote', field2: '', printIfZero: false},

  ];

  const colorBands = [
    {cols: ['I','W'], color: 'FDE9D9'},
    {cols: ['L','M','N','O','P','Q','S'], color: 'E4DFEC'},
    {cols: ['R','T'], color: 'EBF1DE'},
    {cols: ['U'], color: 'DAEEF3'},
  ];
  ...
  for (var j=0; j<pData.length; j++) {
    ...
    // new car hire group
    if (pData[j].CarHireGroups_id !== prevCarHireGroups_id) {
      if (pData[j].States_id === prevStates_id) {
        row += 2;
      } 
      cell = worksheet.getCell('B' + row.toString());
      cell.value = pData[j].CarHireGroup;  
      cell.font = { name: 'Calibri', size: 12, bold: true };    
      row++;
    }

    // new agent
    if (pData[j].Agents_id !== prevAgents_id || pData[j].CarHireGroups_id !== prevCarHireGroups_id) {
    ...
    for (i=0; i<columns.length; i++) {
      ...
      if (columns[i].field2 !== undefined) {
        xfield = (reportObj.id === 62) ? columns[i].field2 : columns[i].field;
      }
    ...
    prevCarHireGroups_id = pData[j].CarHireGroups_id;
