...
import { SERVICE_PER_KM } from '../../../../actions/types';

import './CarPerKm.css'

let compVar = {};

function CarPerKmParams(props) {
  ...
  const setAgentLookup = async() => {
    ...
    compVar.agentLookup =  await getAgentListing(SERVICE_PER_KM.toString(),active);
    setAgent();
  }
  ...
  const setServiceCityLookup = async() => {
    ...
    if (compVar.serviceCitySwitchValue) {
      ...
      tableStr = "cities c WHERE c.cities_id IN " + 
        "(SELECT ch.ServiceCities_id FROM CarHire ch WHERE ch.addressbook_id = " + compVar.agents_id.toString() + ")";    
      compVar.serviceCityLookup = await dbGetRecord({fields: ["cities_id, city"], orders: ['city'], table: tableStr, x_uid: _g_users_id, x_module: 'Cost Car Per Km'});   
    } 
    setServiceCity();
  }
  ...
  const setWefLookup = async() => {
    ...
    compVar.wefLookup = await dbGetRecord({fields: ["DISTINCT Wef, CONVERT(varchar(10),wef,103)  + '-' + CASE WHEN wet IS NOT NULL THEN CONVERT(varchar(10),wet,103) ELSE '' END AS DateRange"], orders: ['Wef DESC'], table: 'CarHire', where: whereStr, x_uid: _g_users_id, x_module: 'Cost Car Per Km'});   
  ...
  const getSelectedParams = async (paramObj) => {
    ...
    dispatch(setParamValues({
      carPerKmAgents_id: compVar.agents_id,
      carPerKmServiceCities_id: compVar.serviceCities_id,
      carPerKmWef: compVar.wef
    }));
  ...
  const renderContent = () => {
    ...
    if (!initDataFetched) {
      return (
        <div className="carperkm-panelparams-container" style={{width: '100%'}}>
    ...
    return (
      <div className="carperkm-panelparams-container" style={{width: '100%'}}>
        ...
        <div className="carperkm-panelparams-section-container">

          <div className="carperkm-panelparams-city-container" style={{flex: 1}}>
            {dropDownParamsJsx(0)}
            {switchParamsJsx(0)}
          </div>
          <div className="carperkm-panelparams-city-container" style={{flex: 1}}>
