...
import DataGrid, {Column,Editing,Paging, KeyboardNavigation} from 'devextreme-react/data-grid';
...
function CostQuickEntry(props) {
  ...
  const columnsJsx = () => {
    ...
    return props.headerData.map(rec => {
      return (
        <Column key={rec.field} dataField={rec.field} caption={rec.caption} width={rec.width} allowEditing={rec.allowEditing} visible={rec.visible} />
      )
    })  
  ...
  return (
    <>
      <Popup visible={true} height={compVar.popupFormHeight} width={compVar.totalColumnWidth+100} onHiding={closePopover}>
