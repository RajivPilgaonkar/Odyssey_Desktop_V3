...
import { getHotelLabel } from "../../../../common/CostingHelpers";
...
function CostAccTourLeader(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      mainTitle: 'Tour Leader', title: 'New Tour Leader',
      ...
      admLevel: 1,
      dbLookup: [
        {keyField: 'PolicyOnEscorts_id', dataSource: compVar.escortPolicyLookup, 
  ...
  const fetchInitialData = async() => {
    ...
    try {
      ...
      compVar.escortPolicyLookup = await dbGetRecord({fields: ['policyonescorts_id', 'policy'], orders: ['policy'], table: 'policyonescorts', x_uid: _g_users_id, x_module: 'Cost Accommodation'});   
      compVar.dbLookup[0].dataSource = compVar.escortPolicyLookup;  

      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Cost Accommodation'});   
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['Seasons_id'], table: 'Seasons', where: whereStr, x_uid: _g_users_id, x_module: 'Cost Accommodation'});   
  ...
  const deleteRow = async (e) => {
    compVar.errorMsg = "Please delete the main Seasons record."
    forceRender();
  }
  ...
  //**********************************************************/
  const getSelectedPolicy = async(e) => {
    compVar.formData.PolicyOnEscorts_id = e[0].policyonescorts_id;
  }
  ...
  //**********************************************************/
  const clearPolicyLookup = async() => {
    compVar.formData.PolicyOnEscorts_id = null;
  }
  ...
  const createFormObject = () => {
    ...
    const initialPolicyLookupValues = getLookupValues (
      clearPolicyLookupValues, compVar.escortPolicyLookup, 
      ['policyonescorts_id','policy'], compVar.formData.PolicyOnEscorts_id);
    ...
    return {...defaultFormObject,
      ...
      //formHelp: formHelp,
      clearLookup: [clearPolicyLookup, clearUserLookup],
      getSelectedRecord: [getSelectedPolicy, getSelectedUser],
...
export default CostAccTourLeader;
