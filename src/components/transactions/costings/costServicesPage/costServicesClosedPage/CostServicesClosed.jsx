...
import { convert_DbDate_To_MDY, getFieldsArray, beforeInsert, saveEditedInsertedData, checkNullErrors, getLookupValues, convertToMoment_fmt } from "../../../../common/CommonTransactionFunctions";
...
import { setDefaultRoom, getServiceLabel} from "../../../../common/CostingHelpers";
import {popupTitleContainerStyle} from "../../../../common/ComponentStyles";
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetServicesClosedData";
...
function CostServicesClosed(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      tableName: 'CostServicesClose', keyField: 'CostServicesClose_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Closed On', title: 'New ... Closed On',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['FromDate'], table: 'CostServicesClose', where: whereStr, x_uid: _g_users_id, x_module: 'Cost Services'});   

      compVar.mainTitle = '[' + props.service.substring(0,25) + '] Closed On'; 
      compVar.title = 'New ... [' + props.service.substring(0,25) + '] Closed On';
  ...
  const editRow = async (e) => {
    ...
    compVar.formTitle = 'Closed On ... ' + title;
    toggleEditPopup();    
  }
  ...
  const addRow = async () => {
    ...
    const title = await getServiceLabel(props.services_id, props.wef);
    compVar.formTitle = 'New Closed On ... ' + title;
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE CostServices_id = " + props.costServices_id.toString() + " " + 
      "AND FromDate = '" + convert_DbDate_To_MDY(compVar.formData.FromDate,1) + "' " + 
      "AND ToDate = '" + convert_DbDate_To_MDY(compVar.formData.ToDate,1) + "' ";
...
export default CostServicesClosed;
