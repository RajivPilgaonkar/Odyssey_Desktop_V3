...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetServicesTransportData";
...
function CostServicesTransport(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      isCostServiceTransportModified: false,
      admLevel: 1,
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainTitle = '[' + props.service + '] Transport Costs'; 
      compVar.title = 'New ... [' + props.service + '] Transport Costs';
  ...
  const saveFormData = async () => {
    ...
    compVar.isCostServiceTransportModified = true;

    // In this case, focus back on the transport grid (which is right at the bottom)
    if (gridRef.current) {
      const el = document.getElementById('gridContainer_transport');
    ...
    /*=== transport cost modified, also refresh meet & assist costs===*/
    if (props.onModifyCostServiceTransport !== undefined) {
      await props.onModifyCostServiceTransport({refresh: compVar.isCostServiceTransportModified});
    }
  ...
  const quickCostEntry = async () => {
    ...
    const query = "SELECT cst.CostServicesTransport_id, cst.FromPax, cst.ToPax, v.Vehicle, " + 
      "COALESCE(cst.CostAc,0) AS CostAc, COALESCE(cst.ParkingFee,0) AS ParkingFee, " +
      "COALESCE(cst.RoadTaxPerDay,0) AS RoadTaxPerDay, COALESCE(cst.MeetAndAssist,0) AS MeetAndAssist, " + 
      "COALESCE(cst.EntryAp,0) AS EntryAp " +
      "FROM CostServicesTransport cst LEFT JOIN vehicles v ON cst.vehicles_id = v.vehicles_id " +
      "WHERE CostService_id = " + props.costServices_id.toString() + " " +
      "ORDER BY cst.FromPax, cst.ToPax";

    compVar.quickEntryData = await dbGetRecordRaw({query: query});

    compVar.quickEntryHeaderData = [
      {field: 'CostServicesTransport_id', caption: 'ID', allowEditing: false, width: 60, visible: false, dataType: 'number'},
      ...
      {field: 'Vehicle', caption: 'Vehicle', allowEditing: false, width: 100, visible: true, dataType: 'string'},
      {field: 'CostAc', caption: 'Cost', allowEditing: true, width: 100, visible: true, dataType: 'number'},
      {field: 'ParkingFee', caption: 'Parking', allowEditing: true, width: 100, visible: true, dataType: 'number'},
      {field: 'RoadTaxPerDay', caption: 'Road Tax', allowEditing: true, width: 100, visible: true, dataType: 'number'},
      {field: 'MeetAndAssist', caption: 'Meet & Assist', allowEditing: true, width: 100, visible: true, dataType: 'number', ma: true},
      {field: 'EntryAp', caption: 'Entry A/P', allowEditing: true, width: 100, visible: true, dataType: 'number', ma: true},
    ];

    /*=== Display M&A only for transfers ===*/    
    compVar.quickEntryHeaderData = (props.transfer) ? 
      compVar.quickEntryHeaderData :
      compVar.quickEntryHeaderData.filter (rec => rec.ma === undefined || !rec.ma);      
        ...
        <div className="master-grid-container" style={{}}>
          ...
          {compVar.displayQuickCost &&
            <CostQuickEntry
              ...
              tableName={'CostServicesTransport'}
              keyField={'CostServicesTransport_id'}
...
export default CostServicesTransport;
