...
import { convert_DbDate_To_MDY, getFieldsArray, beforeInsert, saveEditedInsertedData, checkNullErrors, getLookupValues, escapeSingleQuotes } from "../../../../common/CommonTransactionFunctions";
...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetServicesMiscData";
...
function CostServicesMisc(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      currencyLookup: [], residentLookup: [],   
      tableName: 'CostServicesOthers', keyField: 'CostServicesOthers_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: '[' + props.service + '] Misc Costs', title: 'New ... [' + props.service + '] Misc Costs',
  ...
  const filterData = async() => {
    ...
    try {
      const whereStr = `CostServices_id = ${props.costServices_id.toString()} `;
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['FromPax, ToPax'], table: 'CostServicesOthers', where: whereStr, x_uid: _g_users_id, x_module: 'Cost Services'});   

      let taxStr = '';
      if (props.wef !== undefined && props.wef !== null && props.wef.trim().length > 0) {
        const taxObj = await getCentralTax(props.wef, TAX_ID_MISC);
      ...
      compVar.mainTitle = '[' + props.service + '] Misc Costs'; 
      compVar.title = 'New ... [' + props.service + '] Misc Costs';
  ...
  const addRow = async () => {
    ...
    defaultObj = {...defaultObj, 
      CostServices_id: props.costServices_id
    }

    afterAdd(compVar, defaultObj);
    const title = await getServiceLabel(props.services_id, props.wef);
    compVar.formTitle = 'New Misc ... ' + title;
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE CostServices_id = " + props.costServices_id.toString() + " " + 
      "AND FromPax = " + compVar.formData.FromPax.toString() + " " + 
      "AND ToPax = " + compVar.formData.ToPax.toString() + " " + 
      "AND Resident = " + compVar.formData.Resident.toString() + " " +
      "AND COALESCE(Remarks,'') = '" + (compVar.formData.Remarks !== null ? escapeSingleQuotes(compVar.formData.Remarks) : '') + "' " +
    ...
    // In this case, focus back on the misc grid
    if (gridRef.current) {
      const el = document.getElementById('gridContainer_misc');
  ...
  const quickCostEntry = async () => {
    ...
    const query = "SELECT cso.CostServicesOthers_id, cso.FromPax, cso.ToPax, " + 
      "r.Resident, cso.Remarks, COALESCE(cso.CostPerPerson,0.0) AS CostPerPerson, " + 
      "COALESCE(cso.CostPerGroup,0.0) AS CostPerGroup " +
      "FROM CostServicesOthers cso LEFT JOIN Residents r ON cso.resident = r.residents_id " +
      "WHERE CostServices_id = " + props.costServices_id.toString() + " " +
      "ORDER BY cso.FromPax, cso.ToPax, cso.Remarks, r.Resident ";
    
    compVar.quickEntryData = await dbGetRecordRaw({query: query});

    compVar.quickEntryHeaderData = [
      {field: 'CostServicesOthers_id', caption: 'ID', allowEditing: false, width: 60, visible: false, dataType: 'number'},
      {field: 'FromPax', caption: 'From Pax', allowEditing: false, width: 80, visible: true, dataType: 'number'},
      {field: 'ToPax', caption: 'To Pax', allowEditing: false, width: 80, visible: true, dataType: 'number'},
      {field: 'Resident', caption: 'Resident', allowEditing: false, width: 100, visible: true, dataType: 'string'},
      {field: 'Remarks', caption: 'Remarks', allowEditing: false, width: 250, visible: true, dataType: 'string'},
      {field: 'CostPerPerson', caption: 'Per Person', allowEditing: true, width: 100, visible: true, dataType: 'number'},
      {field: 'CostPerGroup', caption: 'Per Group', allowEditing: true, width: 100, visible: true, dataType: 'number'},
    ];
  ...
  const createFormObject = () => {
    ...
    return {...defaultFormObject,
      ...
      //formHelp: formHelp,
      clearLookup: [clearCurrencyLookup, clearResidentLookup, clearUserLookup],
      getSelectedRecord: [getSelectedCurrency, getSelectedResident, getSelectedUser],
  ...
  const createElementProps = () => {
    ...
    const canAdd = (props.costServices_id !== undefined && props.costServices_id !== null && props.costServices_id > 0);
        ...
        <div className="master-grid-container" style={{}}>
          ...
          {compVar.displayQuickCost &&
            <CostQuickEntry
              ...
              tableName={'CostServicesOthers'}
              keyField={'CostServicesOthers_id'}
...
export default CostServicesMisc;
