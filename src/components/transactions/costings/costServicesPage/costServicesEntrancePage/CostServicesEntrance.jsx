...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetServicesEntranceData";
...
import { TAX_ID_ENTRANCE } from '../../../../../actions/types';
...
function CostServicesEntrance(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      tableName: 'CostServicesEntranceFees', keyField: 'CostServicesEntranceFees_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: '[' + props.service + '] Entrance Costs', title: 'New ... [' + props.service + '] Entrance Costs',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['FromPax, ToPax'], table: 'CostServicesEntranceFees', where: whereStr, x_uid: _g_users_id, x_module: 'Cost Services'});   

      let taxStr = '';
      if (props.wef !== undefined && props.wef !== null && props.wef.trim().length > 0) {
        const taxObj = await getCentralTax(props.wef, TAX_ID_ENTRANCE);
      ...
      compVar.mainTitle = '[' + props.service + '] Entrance Costs'; 
      compVar.title = 'New ... [' + props.service + '] Entrance Costs';
  ...
  const editRow = async (e) => {
    ...
    compVar.formTitle = 'Entrance ... ' + title;
  ...
  const addRow = async () => {
    ...
    const title = await getServiceLabel(props.services_id, props.wef);
    compVar.formTitle = 'New Entrance ... ' + title;
  ...
  const saveFormData = async () => {
    ...
    // In this case, focus back on the entrance grid
    if (gridRef.current) {
      const el = document.getElementById('gridContainer_entrance');
  ...
  const quickCostEntry = async () => {
    ...
    const query = "SELECT cse.CostServicesEntranceFees_id, cse.FromPax, cse.ToPax, " + 
      "r.Resident, cse.Remarks, COALESCE(cse.Cost,0.0) AS Cost " +
      "FROM CostServicesEntranceFees cse LEFT JOIN Residents r ON cse.resident = r.residents_id " +
      "WHERE CostServices_id = " + props.costServices_id.toString() + " " +
      "ORDER BY cse.FromPax, cse.ToPax, cse.Remarks, r.Resident ";
    
    compVar.quickEntryData = await dbGetRecordRaw({query: query});

    compVar.quickEntryHeaderData = [
      {field: 'CostServicesEntranceFees_id', caption: 'ID', allowEditing: false, width: 60, visible: false, dataType: 'number'},
        ...
        <div className="master-grid-container" style={{}}>
          ...
          {compVar.displayQuickCost &&
            <CostQuickEntry
              ...
              tableName={'CostServicesEntranceFees'}
              keyField={'CostServicesEntranceFees_id'}
...
export default CostServicesEntrance;
