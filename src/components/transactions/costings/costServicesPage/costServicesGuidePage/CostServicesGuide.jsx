...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetServicesGuideData";
...
import { TAX_ID_GUIDE } from '../../../../../actions/types';
...
function CostServicesGuide(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      tableName: 'CostServicesGuides', keyField: 'CostServicesGuides_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: '[' + props.service + '] Guide Costs', title: 'New ... [' + props.service + '] Guide Costs',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['FromPax, ToPax'], table: 'CostServicesGuides', where: whereStr, x_uid: _g_users_id, x_module: 'Cost Services'});   

      let taxStr = '';
      if (props.wef !== undefined && props.wef !== null && props.wef.trim().length > 0) {
        const taxObj = await getCentralTax(props.wef, TAX_ID_GUIDE);
      ...
      compVar.mainTitle = '[' + props.service + '] Guide Costs'; 
      compVar.title = 'New ... [' + props.service + '] Guide Costs';
  ...
  const addRow = async () => {
    ...
    const title = await getServiceLabel(props.services_id, props.wef);
    compVar.formTitle = 'New Guide ... ' + title;
  ...
  const saveFormData = async () => {
    ...
    // In this case, focus back on the guide grid
    if (gridRef.current) {
      const el = document.getElementById('gridContainer_guide');
  ...
  const quickCostEntry = async () => {
    ...
    const query = "SELECT csg.CostServicesGuides_id, csg.FromPax, csg.ToPax, " + 
      "r.Resident, csg.Remarks, COALESCE(csg.Cost,0.0) AS Cost " +
      "FROM CostServicesGuides csg LEFT JOIN Residents r ON csg.resident = r.residents_id " +
      "WHERE CostServices_id = " + props.costServices_id.toString() + " " +
      "ORDER BY csg.FromPax, csg.ToPax, csg.Remarks, r.Resident ";
    
    compVar.quickEntryData = await dbGetRecordRaw({query: query});

    compVar.quickEntryHeaderData = [
      {field: 'CostServicesGuides_id', caption: 'ID', allowEditing: false, width: 60, visible: false, dataType: 'number'},
        ...
        <div className="master-grid-container" style={{}}>
          ...
          {compVar.displayQuickCost &&
            <CostQuickEntry
              ...
              tableName={'CostServicesGuides'}
              keyField={'CostServicesGuides_id'}
...
export default CostServicesGuide;
