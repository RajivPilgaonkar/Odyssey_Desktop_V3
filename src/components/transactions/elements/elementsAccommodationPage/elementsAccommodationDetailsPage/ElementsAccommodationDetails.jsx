...
import { getDevExtremeTable, tableHeaderArray, getDevExtremePopupForm } from "./GetElementsAccommodationDetailsData";
...
function ElementsAccommodationDetails(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      mainData: [], userLookup: [],
      tableName: 'ElemAccommodationCosts', keyField: 'ElemAccommodationCosts_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Accommodation Costs', title: 'New Accommodation Cost',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      const whereStr = "(ElemAccommodation_id = " + props.elements_id + ") ";

      compVar.mainData = await dbGetRecord({fields: fieldArray, orders: ['NumPax'], table: compVar.tableName, where: whereStr, x_uid: _g_users_id, x_module: 'Elements Accommodation'});   
  ...
  const addRow = async () => {
    ...
    defaultObj = {...defaultObj, 
      ElemAccommodation_id: props.elements_id
    }
  ...
  const saveFormData = async () => {
    ...
    condition += (compVar.formMode === 2) ? "AND ElemAccommodationCosts_id <> " + compVar.formData.ElemAccommodationCosts_id: "";
...
export default ElementsAccommodationDetails;
