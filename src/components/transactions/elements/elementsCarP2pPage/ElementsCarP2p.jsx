...
import { getDevExtremeTable, tableHeaderArray, getDevExtremePopupForm } from "./GetElementsCarP2pData";
...
function ElementsCarP2p(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      tableName: 'ElemInterCities', keyField: 'ElemInterCities_id',
      masterDescField: '',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      const tableStr = "ElemInterCities ec " + 
      ...
      compVar.mainData = await dbGetRecord({fields: fieldArray, orders: ['c.city,s2.state'], table: tableStr, where: whereStr, x_uid: _g_users_id, x_module: 'Elements Car P2P'});   
  ...
  const saveFormData = async () => {
    ...
    condition += (compVar.formMode === 2) ? "AND ElemInterCities_id <> " + compVar.formData.ElemInterCities_id: "";
  ...
  const setAsQuoted = async (e) => {
    ...
    let sql = "UPDATE ElemInterCities SET Quoted = " + quoted + " " +
      "WHERE ElemInterCities_id = " + e.ElemInterCities_id.toString();      
    let spData = {sql: sql}
  ...
  const quickCostEntry = async () => {
    ...
    const query = "SELECT ec.ElemInterCitiesCosts_id,  " + 
      "ec.NumPax, ec.Cost, ec.TransportCost, ec.RepCost, ec.GuideCost, v.Vehicle " +
      "FROM ElemInterCitiesCosts ec " +
      "LEFT JOIN Vehicles v ON ec.Vehicles_id = v.Vehicles_id " +
      "WHERE ec.ElemInterCities_id = " + compVar.focusedRowKey.toString() + " " +
      "ORDER BY ec.NumPax ";
    
    compVar.quickEntryData = await dbGetRecordRaw({query: query});

    compVar.quickEntryHeaderData = [
      {field: 'ElemInterCitiesCosts_id', caption: 'ID', allowEditing: false, width: 60, visible: false, dataType: 'number'},
      ...
      {field: 'RepCost', caption: 'Rep', allowEditing: true, width: 100, visible: true, dataType: 'number'},
      {field: 'GuideCost', caption: 'Guide', allowEditing: true, width: 100, visible: true, dataType: 'number'},
    ];

    compVar.sqlTotal = "UPDATE ElemInterCitiesCosts SET Cost = TransportCost + RepCost + GuideCost ";
  ...
  //**********************************************************/
  const elementDeleteOneToTenCosts = async (elemInterCities_id) => {

    let sql = "DELETE FROM ElemInterCitiesCosts WHERE ElemInterCities_id = " + 
      elemInterCities_id.toString();
    let spData = {sql: sql}
    await dbExecuteSp(spData);
        ...
        <div className="master-grid-container" style={{height: containerHeight, justifyContent: 'flex-start'}}>
          ...
          {compVar.displayQuickCost &&
            <CostQuickEntry
              ...
              tableName={'ElemInterCitiesCosts'}
              keyField={'ElemInterCitiesCosts_id'}
