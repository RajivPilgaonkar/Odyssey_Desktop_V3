...
import { getDevExtremeTable, tableHeaderArray, getDevExtremePopupForm } from "./GetElementsCarCityGroupsData";
...
function ElementsCarCityGroups(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      tableName: 'ElemCityGroups', keyField: 'ElemCityGroups_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Car City Groups', title: 'New Car City Group',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      const tableStr = "ElemCityGroups ec " + 
      ...
      compVar.mainData = await dbGetRecord({fields: fieldArray, orders: ['c.city,s2.state'], table: tableStr, where: whereStr, x_uid: _g_users_id, x_module: 'Elements Car City Groups'});   
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE Addressbook_id = " + compVar.formData.Addressbook_id + " "  +
      ...
      "AND CarHireGroup = '" + compVar.formData.CarHireGroup + "' ";
    condition += (compVar.formMode === 2) ? "AND ElemCityGroups_id <> " + compVar.formData.ElemCityGroups_id : "";
  ...
  const quickCostEntry = async () => {
    ...
    const query = "SELECT ec.ElemCityGroupsCosts_id,  " + 
      "ec.NumPax, ec.Cost, ec.CostTransport, ec.CostEscort, v.Vehicle " +
      "FROM ElemCityGroupsCosts ec " +
      "LEFT JOIN Vehicles v ON ec.Vehicles_id = v.Vehicles_id " +
      "WHERE ec.ElemCityGroups_id = " + compVar.focusedRowKey.toString() + " " +
      "ORDER BY ec.NumPax ";
    
    compVar.quickEntryData = await dbGetRecordRaw({query: query});

    compVar.quickEntryHeaderData = [
      {field: 'ElemCityGroupsCosts_id', caption: 'ID', allowEditing: false, width: 60, visible: false, dataType: 'number'},
      ...
      {field: 'CostTransport', caption: 'Transport', allowEditing: true, width: 100, visible: true, dataType: 'number'},
      {field: 'CostEscort', caption: 'Escort', allowEditing: true, width: 100, visible: true, dataType: 'number'},
    ];

    compVar.sqlTotal = "UPDATE ElemCityGroupsCosts SET Cost = CostTransport + CostEscort ";
  ...
  const setAsQuoted = async (e) => {
    ...
    let sql = "UPDATE ElemCityGroups SET Quoted = " + quoted + " " +
      "WHERE ElemCityGroups_id = " + e.ElemCityGroups_id.toString();      
    let spData = {sql: sql}
  ...
  const elementOneToTenCosts = async () => {
    ...
    // No manual entries allowed anyway
    //let sql = "EXEC [p_ElemInterCityFillZeroCost] " + compVar.focusedRowKey.toString() + ", 1";
    //let spData = {sql: sql}
    //await dbExecuteSp(spData);
  ...
  //**********************************************************/
  const elementDeleteOneToTenCosts = async (elemCityGroups_id) => {

    let sql = "DELETE FROM ElemCityGroupsCosts WHERE ElemCityGroups_id = " + 
      elemCityGroups_id.toString();
    let spData = {sql: sql}
    await dbExecuteSp(spData);
        ...
        <div className="master-grid-container" style={{height: containerHeight, justifyContent: 'flex-start'}}>
          ...
          {compVar.displayQuickCost &&
            <CostQuickEntry
              ...
              tableName={'ElemCityGroupsCosts'}
              keyField={'ElemCityGroupsCosts_id'}
...
export default ElementsCarCityGroups;
