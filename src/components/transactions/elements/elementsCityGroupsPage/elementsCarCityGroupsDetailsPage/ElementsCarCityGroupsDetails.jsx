...
import { getDevExtremeTable, tableHeaderArray, getDevExtremePopupForm } from "./GetElementsCarCityGroupsDetailsData";
...
function ElementsCarCityGroupsDetails(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      mainData: [], userLookup: [],
      vehicleLookup: [], 
      tableName: 'ElemCityGroupsCosts', keyField: 'ElemCityGroupsCosts_id',
      masterDescField: '',
  ...
  const fetchInitialData = async() => {
    ...
    try {
      ...
      let whereStr = "vehicles_id IN " + 
        "(SELECT vehicles_id FROM CarHireGroupCosts ch " + 
  ...
  const filterData = async() => {
    ...
    try {
      ...
      const whereStr = "(ElemCityGroups_id = " + props.elements_id + ") ";

      compVar.mainData = await dbGetRecord({fields: fieldArray, orders: ['NumPax'], table: compVar.tableName, where: whereStr, x_uid: _g_users_id, x_module: 'Elements City Groups'});   
  ...
  const addRow = async () => {
    ...
    defaultObj = {...defaultObj, 
      ElemCityGroups_id: props.elements_id
    }
  ...
  const saveFormData = async () => {
    ...
    condition += (compVar.formMode === 2) ? "AND ElemCityGroupsCosts_id <> " + compVar.formData.ElemCityGroupsCosts_id : "";
  ...
  const onFormFieldDataChanged = (e) => {
    ...
    if ((e.dataField === 'CostTransport') || 
        (e.dataField === 'CostEscort')) {
  ...
  const computeLineTotals = () => {
    ...
    const transportCost = (compVar.formData.CostTransport !== null) ? compVar.formData.CostTransport : 0;
    const escortCost = (compVar.formData.CostEscort !== null) ? compVar.formData.CostEscort : 0;

    const total = transportCost + escortCost;
...
export default ElementsCarCityGroupsDetails;
