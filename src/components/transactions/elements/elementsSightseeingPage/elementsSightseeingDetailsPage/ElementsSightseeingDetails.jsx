...
import { convert_DbDate_To_MDY, beforeInsert, saveEditedInsertedData, checkNullErrors, getFieldsArray, getLookupValues } from "../../../../common/CommonTransactionFunctions";
...
import { getDevExtremeTable, tableHeaderArray, getDevExtremePopupForm } from "./GetElementsSightseeingDetailsData";
...
function ElementsSightseeingDetails(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      mainData: [], userLookup: [], vehicleLookup: [],
      tableName: 'ElemServicesCosts', keyField: 'ElemServicesCosts_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Sightseeing Costs', title: 'New Sightseeing Cost',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      const whereStr = "(ElemServices_id = " + props.elements_id + ") ";

      compVar.mainData = await dbGetRecord({fields: fieldArray, orders: ['NumPax'], table: compVar.tableName, where: whereStr, x_uid: _g_users_id, x_module: 'Elements Services'});   
  ...
  const addRow = async () => {
    ...
    defaultObj = {...defaultObj, 
      ElemServices_id: props.elements_id
    }
  ...
  const saveFormData = async () => {
    ...
    condition += (compVar.formMode === 2) ? "AND ElemServicesCosts_id <> " + compVar.formData.ElemServicesCosts_id: "";
  ...
  const onFormFieldDataChanged = (e) => {
    ...
    if ((e.dataField === 'TransportCost') || 
        (e.dataField === 'MiscCost') ||
        (e.dataField === 'GuideCost') ||
        (e.dataField === 'EntranceFees')) {
  ...
  const computeLineTotals = () => {
    ...
    const miscCost = (compVar.formData.MiscCost !== null) ? compVar.formData.MiscCost : 0;
    const guideCost = (compVar.formData.GuideCost !== null) ? compVar.formData.GuideCost : 0;
    const entranceCost = (compVar.formData.EntranceFees !== null) ? compVar.formData.EntranceFees : 0;

    const total = transportCost + miscCost + guideCost + entranceCost;
  ...
  const createFormObject = () => {
    ...
    return {...defaultFormObject,
      ...
      clearLookupValues: [clearUserLookupValues, clearUserLookupValues],
...
export default ElementsSightseeingDetails;
