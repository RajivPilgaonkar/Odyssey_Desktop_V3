...
import { getExtraDaySectorFromCities } from "../../../common/GetDescFromIds";
...
import { getDevExtremeTable, tableHeaderArray, getDevExtremePopupForm } from "./GetElementsCarExtraDayData";
...
function ElementsCarExtraDay(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      mainData: [], userLookup: [],
      cityLookup: [], agentLookup: [], currencyLookup: [], 
      wef: props.wef, elementType: props.elementType, elementLabel: props.elementLabel,
      tableName: 'ElemExtraDayCarHire', keyField: 'ElemExtraDayCarHire_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Car Extra Day', title: 'New Car Extra Day',
  ...
  const fetchInitialData = async() => {
    ...
    try {
      ...
      compVar.agentLookup = await getAgentServicesListing('2', false);
  ...
  const filterData = async() => {
    ...
    try {
      ...
      const tableStr = "ElemExtraDayCarHire ec " + 
        "LEFT JOIN Addressbook a ON ec.Addressbook_id = a.Addressbook_id " +
        "LEFT JOIN Cities c ON ec.Cities_id = c.Cities_id " +
        "LEFT JOIN States s2 ON c.States_id = s2.States_id ";
  
      compVar.mainData = await dbGetRecord({fields: fieldArray, orders: ['c.city,s2.state'], table: tableStr, where: whereStr, x_uid: _g_users_id, x_module: 'Elements Car Extra Day'});   
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE Addressbook_id = " + compVar.formData.Addressbook_id + " "  +
      "AND Wef = '" + wef + "' " + 
      "AND Cities_id = " + compVar.formData.Cities_id; 
    condition += (compVar.formMode === 2) ? "AND ElemExtraDayCarHire_id <> " + compVar.formData.ElemExtraDayCarHire_id: "";
  ...
  const getSelectedCity = async(e) => {
    ...
    compVar.formData.Sector = await getExtraDaySectorFromCities(compVar.formData.Cities_id);
    forceRender();
  }
  ...
  const setAsQuoted = async (e) => {
    ...
    let sql = "UPDATE ElemExtraDayCarHire SET Quoted = " + quoted + " " +
      "WHERE ElemExtraDayCarHire_id = " + e.ElemExtraDayCarHire_id.toString();      
    let spData = {sql: sql}
  ...
  const quickCostEntry = async () => {
    ...
    const query = "SELECT ec.ElemExtraDayCarHireCosts_id,  " + 
      "ec.NumPax, ec.Cost, v.Vehicle " +
      "FROM ElemExtraDayCarHireCosts ec " +
      "LEFT JOIN Vehicles v ON ec.Vehicles_id = v.Vehicles_id " +
      "WHERE ec.ElemExtraDayCarHire_id = " + compVar.focusedRowKey.toString() + " " +
      "ORDER BY ec.NumPax ";
    
    compVar.quickEntryData = await dbGetRecordRaw({query: query});

    compVar.quickEntryHeaderData = [
      {field: 'ElemExtraDayCarHireCosts_id', caption: 'ID', allowEditing: false, width: 60, visible: false, dataType: 'number'},
  ...
  const elementOneToTenCosts = async () => {
    ...
    let sql = "EXEC [p_ElemExtraDayCarHireFillZeroCost] " + compVar.focusedRowKey.toString() + ", 1";
    let spData = {sql: sql}
    await dbExecuteSp(spData);
  ...
  //**********************************************************/
  const elementDeleteOneToTenCosts = async (elemExtraDayCarHire_id) => {

    let sql = "DELETE FROM ElemExtraDayCarHireCosts WHERE ElemExtraDayCarHire_id = " + 
      elemExtraDayCarHire_id.toString();
    let spData = {sql: sql}
    await dbExecuteSp(spData);
  ...
  const createFormObject = () => {
    ...
    return {...defaultFormObject,
      ...
      formHelp: formHelp,
      clearLookup: [clearCityLookup, clearAgentLookup, clearCurrencyLookup, clearCurrencyLookup, clearUserLookup],
      getSelectedRecord: [getSelectedCity, getSelectedAgent, getSelectedCurrency, getSelectedUser],
      initialLookupValues: [initialCityLookupValues, initialAgentLookupValues, initialCurrencyLookupValues, initialUserLookupValues],
      clearLookupValues: [clearCityLookupValues, clearAgentLookupValues, clearCurrencyLookupValues, clearUserLookupValues],
        ...
        <div className="master-grid-container" style={{height: containerHeight, justifyContent: 'flex-start'}}>
          ...
          {compVar.displayQuickCost &&
            <CostQuickEntry
              ...
              tableName={'ElemExtraDayCarHireCosts'}
              keyField={'ElemExtraDayCarHireCosts_id'}
...
export default ElementsCarExtraDay;
