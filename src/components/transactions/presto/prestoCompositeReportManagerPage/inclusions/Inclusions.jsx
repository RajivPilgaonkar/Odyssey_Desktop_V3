...
import { dbGetRecord, dbExecuteSp, dbDeleteRecord } from '../../../../../actions';
import { beforeInsert, saveEditedInsertedData, checkNullErrors, saveReordedListToDB, getReorderedList, getFieldsArray, getLookupValues } from "../../../../common/CommonTransactionFunctions";
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetInclusionData";
...
function Inclusions(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'QuoPrintInclusions', keyField: 'QuoPrintInclusions_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Inclusions', title: 'New Item',
      ...
      formHeight: 410,
      popupDialogIndex: 0, popupSelectedOptions: [getPopupSelectedOption,setStopNavigation,recreateInclusionProc],
      ...
      dbLookup: [       
        {keyField: 'types_id', dataSource: [{types_id: 1, type: 'Accommodation'}, {types_id: 2, type: 'Tickets'}, {types_id: 3, type: 'Transport'}, {types_id: 4, type: 'Services'}], 
        displayExpr: 'type', valueExpr: 'type', fieldList: ['type']},
  ...
  const fetchInitialData = async() => {
    try {
      ...
      //compVar.dbLookup[0].dataSource = compVar.dbLookup[0].dataSource;  

      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Composite Report Inclusions'});   
  ...
  const filterData = async() => {
    ...
    try {
      const whereStr = "QuoPrint_id = " + props.quoPrint_id.toString();
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo, SrNo'], table: 'QuoPrintInclusions', where: whereStr, x_uid: _g_users_id, x_module: 'Aircraft Types'});   
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('QuoPrintInclusions','SrNo','QuoPrint_id = ' + props.quoPrint_id.toString());
  ...
  //**********************************************************/
  const getSelectedType = async(e) => {
    compVar.formData.ServiceType = e[0].type;
    compVar.formData.OrderNo = e[0].types_id;
    forceRender();
  }

  //**********************************************************/
  const clearSelectedType = async() => {
    compVar.formData.ServiceType = null;
    compVar.formData.OrderNo = null;        
  }
  ...
  //**********************************************************/
  const recreateInclusion = async () => {
    compVar.popupDialogIndex = 2;
    compVar.dialogMessage1 = 'This will delete & re-create all the inclusion lines. Are you sure?';
    compVar.dialogMessage2 = '';
    setPopupDialogBoxVisible(() => {return true});
  }

  //**********************************************************/
  const recreateInclusionProc = async (e) => {
    ...
    if (e===1) {
        ...
        const sql = 'EXEC p_QuoInsertPrintItinerary ' + 
          props.quotations_id.toString() + ', 12';
  ...
  const getNextSrOrderNo = async () => {
    ...
    const nextObj = await getNextSrNo('QuoPrintInclusions','SrNo','QuoPrint_id = ' + props.quoPrint_id.toString() + ' AND OrderNo = ' + compVar.formData.OrderNo.toString());
    const nextSrNo = (nextObj.length > 0) ? nextObj[0].OrderNo+1 : 1;
  ...
  const createFormObject = () => {
    ...
    const clearTypeLookupValues = {types_id: null, type: ''};

    const initialTypeLookupValues = getLookupValues (
      clearTypeLookupValues, compVar.dbLookup[0].dataSource, 
      ['type','type'], compVar.formData.ServiceType);
    ...
    return {...defaultFormObject,
      ...
      clearLookup: [clearSelectedType],
      getSelectedRecord: [getSelectedType],
      initialLookupValues: [initialTypeLookupValues],
      clearLookupValues: [clearTypeLookupValues],
  ...
  const createElementProps = () => {
    ...
    return {
      ...
      buttonListObj: [
        ...
        {visible: !rowDragging, options: {icon: "remove", onClick: recreateInclusion, hint: 'Delete and Recreate Inclusions'}},
      ],
