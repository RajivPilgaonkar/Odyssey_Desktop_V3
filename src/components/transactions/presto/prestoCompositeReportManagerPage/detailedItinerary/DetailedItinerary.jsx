...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetDetailedItineraryData";
...
function DetailedItinerary(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'QuoPrintDays', keyField: 'QuoPrintDays_id',
      masterDescField: 'DayInfo',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Detailed Itinerary', title: 'New Item',
      ...
      formHeight: 410,
      popupDialogIndex: 0, popupSelectedOptions: [getPopupSelectedOption,setStopNavigation,recreateItineraryProc],
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['SrNo'], table: 'QuoPrintDays', where: whereStr, x_uid: _g_users_id, x_module: 'Presto Detailed Itinerary'});   
      compVar.clonedMainData =  [...compVar.mainData];     
    } catch(err) {
      alert(err);
    }
    setFocusedRow(compVar);
    compVar.deleteAndRecreate = false;
    await returnId();    
    setDataFetched(true);
  }
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('QuoPrintDays','SrNo','QuoPrint_id = ' + props.quoPrint_id.toString());
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'QuoPrintPlaces', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Places. Delete the places first'},
    ]);    
  ...
  const saveFormData = async () => {
    ...
    // update basic itinerary
    await updateBasicItinerary(saveData.formData);
  ...
  const toggleEditPopup = () => {
    ...
    setEditPopupVisible(prevEditPopupVisible => !prevEditPopupVisible);
  }; 
  ...
  const returnId = async () => {
    ...
    if (props.onQuoPrintDaysChange !== undefined) {
      await props.onQuoPrintDaysChange({
        quoPrintDays_id: compVar.focusedRowKey,
        deleteAndRecreate: compVar.deleteAndRecreate
      });
    };  
}
  ...
  //**********************************************************/
  const updateBasicItinerary = async(data) => {

    const daySummaryInfo = (data.DaySummaryInfo !== null) ? data.DaySummaryInfo : 'null';
    const quoPrint_id = data.QuoPrint_id;
    const srNo = data.SrNo;

    const sql = "UPDATE QuoPrintItineraries SET DaySummaryInfo = '" + daySummaryInfo.replace(/'/g, "''") + "' " +    
      "WHERE QuoPrint_id = " + quoPrint_id.toString() + " " +
      "AND SrNo = " + srNo.toString();
  ...
  //**********************************************************/
  const recreateItinerary = async () => {
    compVar.popupDialogIndex = 2;
    compVar.dialogMessage1 = 'This will delete & re-create all the basic/detailed itinerary lines. Are you sure?';
    compVar.dialogMessage2 = '';
    setPopupDialogBoxVisible(() => {return true});
  }

  //**********************************************************/
  const recreateItineraryProc = async (e) => {
    ...
    if (e===1) {
        ...
        compVar.deleteAndRecreate = true;
        await returnId();    
  ...
  const createElementProps = () => {
    ...
    return {
      ...
      buttonListObj: [
        ...
        {visible: !rowDragging, options: {icon: "remove", onClick: recreateItinerary, hint: 'Delete and Recreate Basic/Detailed Itinerary'}},
      ],
