...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetBasicItineraryData";
...
function BasicItinerary(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'QuoPrintItineraries', keyField: 'QuoPrintItineraries_id',
      masterDescField: 'DayInfo',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Basic Itinerary', title: 'New Item',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Basic Itinerary'});   
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['SrNo'], table: 'QuoPrintItineraries', where: whereStr, x_uid: _g_users_id, x_module: 'Aircraft Types'});   
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('QuoPrintItineraries','SrNo','QuoPrint_id = ' + props.quoPrint_id.toString());
  ...
  const saveFormData = async () => {
    ...
    await updateDetailedItinerary(saveData.formData);
  ...
  //**********************************************************/
  const updateDetailedItinerary = async(data) => {
    ...
    const sql = "UPDATE QuoPrintDays SET DaySummaryInfo = '" + daySummaryInfo.replace(/'/g, "''") + "' " +    
