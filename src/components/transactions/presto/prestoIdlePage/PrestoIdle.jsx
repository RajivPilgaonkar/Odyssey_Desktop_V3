...
import {saveEditedInsertedData, checkNullErrors, getFieldsArray, setDateTimeFormat, beforeInsert, convertToMoment_fmt, convert_DbDate_To_MDY, isValidDateString } from "../../../common/CommonTransactionFunctions";
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetPrestoIdleData";
...
function PrestoIdle(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'QuoIdle', keyField: 'QuoIdle_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Set Idle Period for your Services', title: 'New Record ... Set Idle Period for your Services',
  ...
  const filterData = async() => {
    ...
    fieldArray.push("CONVERT(varchar(5),FromDate,108) AS FromDate_Time");
    fieldArray.push("CONVERT(varchar(5),ToDate,108) AS ToDate_Time");

    try {
      let whereStr = "qi.Quotations_id = " + props.quotations_id.toString();
      const tableStr = "QuoIdle qi";  
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['qi.FromDate'], table: tableStr, where: whereStr, x_uid: _g_users_id, x_module: 'Presto Set Idle Time'});   
  ...
  const addRow = async () => {
    ...
    compVar.formTitle = 'New Set Idle Record  ... ' +  props.pax;
  ...
  const manipulateDataBeforeSave = () => {
    ...
    if (compVar.formData.FromDate !== null && compVar.formData.FromDate_Time !== null) {
      const fromDate = convert_DbDate_To_MDY(compVar.formData.FromDate,1) + ' ' + compVar.formData.FromDate_Time;
      if (!isValidDateString(fromDate)) {
        alert('Invalid From Date ' + fromDate);
        return;
      }
      timing = fromDate;
      compVar.formData.FromDate = timing;  
    } else if (compVar.formData.FromDate_Time === null) {
      timing = convert_DbDate_To_MDY(compVar.formData.FromDate,1) + ' 09:00';
      compVar.formData.FromDate = timing;  
    }

    if (compVar.formData.ToDate !== null && compVar.formData.ToDate_Time !== null) {
      const toDate = convert_DbDate_To_MDY(compVar.formData.ToDate,1) + ' ' + compVar.formData.ToDate_Time;
      if (!isValidDateString(toDate)) {
        alert('Invalid From Date ' + toDate);
        return;
      }
      timing = toDate;
      compVar.formData.ToDate = timing;  
    } else if (this.var.formData.ToDate_Time === null) {
      timing = convert_DbDate_To_MDY(compVar.formData.ToDate,1) + ' 23:00';
      compVar.formData.ToDate = timing;  
    }

    if (!isValidDateString(compVar.formData.ToDate)) {
      alert('Invalid To Date ' + compVar.formData.ToDate);
      return;
    }
