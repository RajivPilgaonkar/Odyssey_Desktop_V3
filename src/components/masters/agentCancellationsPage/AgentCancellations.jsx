...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetAgentCancellationData";
...
function AgentCancellations() {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      selectedAgents_id: -1, selectedAgent: '',  
      tableName: 'AgentCancelPolicy', keyField: 'AgentCancelPolicy_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Agent Cancellation Policy', title: 'New Cancellation Policy',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Agent Cancellation'});   
  ...
  const filterData = async() => {
    ...
    let whereStr = 'Addressbook_id = ' + compVar.selectedAgents_id.toString();
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['wef DESC'], table: 'AgentCancelPolicy', where: whereStr, x_uid: _g_users_id, x_module: 'Agent Cancellation'});   
    } catch(err) {
      alert(err);
    }
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE wef = '" + convert_DbDate_To_MDY(compVar.formData.Wef,1) + "' " + 
      "AND Addressbook_id = " + compVar.formData.Addressbook_id.toString() + " "; 
    ...
    // update Prev wet
    await updatePreviousWet (compVar.formData.wef, compVar.formData.Addressbook_id);
  ...
  const checkFormErrors = async (formData) => {
    ...
    if (compVar.formData.FromDays > compVar.formData.ToDays) {
      return "'From Days' cannot exceed 'To Days'"      
    }
  ...
  const createElementProps = () => {
    ...
    const canAdd = (compVar.selectedAgents_id > 0) ? compVar.canAdd : false;
