...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetTaxRateData";
...
function TaxRates() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      taxLookup: [],
      selectedTaxes_id: -1, selectedTax: '',  
      tableName: 'centraltaxes', keyField: 'centraltaxes_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Tax Rates', title: 'New Tax Rate',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.taxLookup = await dbGetRecord({fields: ['tax', 'taxes_id'], orders: ['tax'], table: 'taxes', x_uid: _g_users_id, x_module: 'Tax Rates'}); 

      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Tax Rates'});   
  ...
  const filterData = async() => {
    ...
    let whereStr = 'taxes_id = ' + compVar.selectedTaxes_id.toString();
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['wef DESC'], table: 'centraltaxes', where: whereStr, x_uid: _g_users_id, x_module: 'Tax Rates'});   
    } catch(err) {
      alert(err);
    }
  ...
  const addRow = async () => {
    ...
    defaultObj = {...defaultObj, 
      taxes_id: compVar.selectedTaxes_id,
    }
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE wef = '" + convert_DbDate_To_MDY(compVar.formData.wef,1) + "' " + 
      "AND taxes_id = " + compVar.formData.taxes_id.toString() + " "; 
    condition += (compVar.formData.taxes_id === 29) ? " AND fromrate = " + compVar.formData.fromrate.toString() + " " : "";
...
console.log('condition',condition);    
    ...
    // update Prev wet
    await updatePreviousWet (compVar.formData.wef, compVar.formData.taxes_id);
  ...
  const createElementProps = () => {
    ...
    const canAdd = (compVar.selectedTaxes_id > 0) ? compVar.canAdd : false;
  ...
  //*********************************************************/
  const onTaxSelect = async(e) => {
    if (e.length > 0) {
      compVar.selectedTaxes_id = e[0].taxes_id;
      compVar.selectedTax = e[0].tax;  
      filterData();
    }
  }
