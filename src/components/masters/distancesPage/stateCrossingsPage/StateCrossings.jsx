...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetStateCrossingData";
...
function StateCrossings(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  stateLookup: [], mainData: [],
      clonedMainData: [],
      tableName: 'StateCrossings', keyField: 'StateCrossings_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Through States', title: 'New State in Route',
  ...
  const fetchInitialData = async() => {    
    ...
    try {
      compVar.stateLookup = await dbGetRecord({fields: ['states_id', 'state'], orders: ['state'], table: 'states', x_uid: _g_users_id, x_module: 'Distances'});   
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo'], table: 'StateCrossings', where: whereStr, x_uid: _g_users_id, x_module: 'Distances'});   
  ...
  const addRow = async () => {
    ...
    const whereStr = 'Distances_id = ' + props.distances_id.toString();
    const nextObj = await getNextSrNo('StateCrossings','OrderNo',whereStr);
  ...
  const createFormObject = () => {
    ...
    return {...defaultFormObject,
      ...
      formHelp: formHelp,
      clearLookup: [clearStateLookup, clearUserLookup],
      getSelectedRecord: [getSelectedState, getSelectedUser],
      initialLookupValues: [initialStateLookupValues, initialUserLookupValues],
      clearLookupValues: [clearStateLookupValues, clearUserLookupValues],
    }
