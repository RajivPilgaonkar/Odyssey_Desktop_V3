...
import { beforeInsert, getLookupValues, saveEditedInsertedData, checkNullErrors, convert_DbDate_To_MDY, getFieldsArray, saveReordedListToDB, getReorderedList, isValidTime } from "../../../common/CommonTransactionFunctions";
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetCityCrossingData";
...
function CityCrossings(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      clonedMainData: [],
      tableName: 'CityCrossings', keyField: 'CityCrossings_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Through Cities', title: 'New City in Route',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      const whereStr = 'Distances_id = ' + props.distances_id.toString();
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo'], table: 'CityCrossings', where: whereStr, x_uid: _g_users_id, x_module: 'Distances'});   
  ...
  const saveFormData = async () => {
    ...
    // Allow at all times
    let condition = "WHERE Distances_id = " + props.distances_id.toString() + " " +
    ...
    // Mark distances table as edited
    await markAsEdited();

    // update in master based on cities in details
    updateViaCities();
  ...
  //**********************************************************/
  const updateViaCities = async () => {

    // update car hire group in master based on cities in details
    const sql = "EXEC [p_ViaCitiesInDistances] " +
      props.distances_id.toString();
    ...
    const whereStr = 'Distances_id = ' + props.distances_id.toString();
    const distancesObj = await dbGetRecord({fields: ['Via'], orders: ['Distances_id'], table: 'Distances', where: whereStr});   

    props.distancesFormObj.formData.via = distancesObj[0].Via;
    props.distancesFormObj.refreshFormData();
  ...
  const markAsEdited = async () => {
    ...
    // update car hire group in master based on cities in details
    const sql = "UPDATE Distances SET edited = 1 WHERE distances_id = " +
      props.distances_id.toString();
    let spData = {sql: sql};
    try {
      await dbExecuteSp(spData);  
  ...
  const renderContent = () => {
    ...
    const containerHeight = heights.containerHeight/2;
    const viewHeight = heights.viewHeight/2;
