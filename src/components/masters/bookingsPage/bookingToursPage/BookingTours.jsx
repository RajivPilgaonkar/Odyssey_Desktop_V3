...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetBookingToursData";
...
function BookingTours(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      clonedMainData: [],
      tableName: 'BookingsTours', keyField: 'BookingsTours_id',
      masterDescField: '',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['TourDate'], table: 'BookingsTours', where: whereStr, x_uid: _g_users_id, x_module: 'Bookings'});   
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE Bookings_id = " + props.bookings_id.toString() + " " + 
      "AND TourCode = '" + compVar.formData.TourCode + "' " + 
      "AND TourDate = '" + convert_DbDate_To_MDY(compVar.formData.TourDate, 1) + "' ";
