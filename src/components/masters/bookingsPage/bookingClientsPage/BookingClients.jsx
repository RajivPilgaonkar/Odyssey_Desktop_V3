...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetBookingClientsData";
...
function BookingClients(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  countryLookup: [], mainData: [],
      clonedMainData: [],
      tableName: 'BookingsClients', keyField: 'BookingsClients_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Pax', title: 'New Pax',
  ...
  const filterData = async() => {
    ...
    try {
      ...
      const whereStr = 'Bookings_id = ' + props.bookings_id.toString();
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['LeadPaxOrder'], table: 'BookingsClients', where: whereStr, x_uid: _g_users_id, x_module: 'Distances'});   
  ...
  const addRow = async () => {
    ...
    const whereStr = 'Bookings_id = ' + props.bookings_id.toString();
    const nextObj = await getNextSrNo('BookingsClients','LeadPaxOrder',whereStr);
    const nextSrNo = (nextObj.length > 0) ? nextObj[0].OrderNo+1 : 1;

    // add other code like next sr no, yearref ...
    defaultObj = {...defaultObj, 
      Bookings_id: props.bookings_id,
      LeadPaxOrder: nextSrNo
    }
  ...
  const saveFormData = async () => {
    ...
    // Allow at all times
    let condition = "WHERE Bookings_id = " + props.bookings_id.toString() + " " + 
  ...
  //**********************************************************/
  const getSelectedResCountry = (e) => {
    compVar.formData.Resident_Countries_id = e[0].countries_id;
  }
  ...
  //**********************************************************/
  const clearResCountryLookup = () => {
    compVar.formData.Resident_Countries_id = null;
  }
  ...
  const createFormObject = () => {
    ...
    const clearResCountryLookupValues = {countries_id: null, country: ''};
    ...
    const initialResCountryLookupValues = getLookupValues(
      clearResCountryLookupValues,compVar.countryLookup, 
      ['countries_id', 'country'], compVar.formData.Resident_Countries_id);
    ...
    return {...defaultFormObject,
      ...
      formHelp: formHelp,
      clearLookup: [clearCountryLookup, clearResCountryLookup, clearUserLookup],
      getSelectedRecord: [getSelectedCountry, getSelectedResCountry, getSelectedUser],
      initialLookupValues: [initialCountryLookupValues, initialResCountryLookupValues, initialUserLookupValues],
      clearLookupValues: [clearCountryLookupValues, clearResCountryLookupValues, clearUserLookupValues],
    }
  ...
  const createElementProps = () => {
    ...
    const canAdd = (compVar.mainData.length > 2);
  ...
  const renderContent = () => {
    ...
    // heights changed here as another state grid has to be accommodated at the bottom
    const heights = getViewContainerHeights(compVar);
    const containerHeight = (heights.containerHeight-60)/2;
    const viewHeight = (heights.viewHeight-60)/2;
    ...
    return (
      <>
        <div className="master-grid-container" style={{height: containerHeight}}>
          ...
          {!editPopupVisible &&
            <div className="master-grid-content-box" style={{height: 220}}>
              {(compVar.errorMsg > '') &&
