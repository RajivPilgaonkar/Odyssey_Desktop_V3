...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetAddressbookHotelData";
...
function AddressbookHotel(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'Hotels', keyField: 'Addressbook_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Hotel', title: 'New Hotel',
  ...
  const filterData = async() => {
    ...
    const spData = {sql: `EXEC p_InsertHotels`, x_uid: props.users_id, x_module: 'Addressbook Subcategories'};
    try {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['Addressbook_id'], table: 'Hotels', where: whereStr, x_uid: _g_users_id, x_module: 'Addressbook Hotels'});   
    } catch(err) {
      alert(err);
    }
  ...
  const checkFormErrors = async (formData) => {
    ...
    if ((formData.CheckIn !== null) && (formData.CheckIn.trim().length > 0) && !isValidTime(formData.CheckIn)) {
      return "Invalid 'Check In' entered";
    }

    if ((formData.CheckOut !== null) && (formData.CheckOut.trim().length > 0) && !isValidTime(formData.CheckOut)) {
      return "Invalid 'Check Out' entered";
    }
...
export default AddressbookHotel;
