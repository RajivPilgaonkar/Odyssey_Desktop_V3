...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetAddressbookContactsData";
...
function AddressbookContacts(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'AddressDetails', keyField: 'AddressDetails_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Contacts', title: 'New Contact',
  ...
  const filterData = async() => {
    ...
    try {
      const whereStr = 'Addressbook_id = ' + props.addressbook_id.toString();
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo'], table: 'AddressDetails', where: whereStr, x_uid: _g_users_id, x_module: 'Addressbook Contacts'});   
    ...
    for (const rec of compVar.mainData) {
      // construct full name
      rec.Name = (rec.FirstName !== null) ? rec.FirstName.trim() : '';
      rec.Name += (rec.Name.length > 0) ? ' ' : '';
      rec.Name += (rec.LastName !== null) ? rec.LastName.trim() : '';

      // construct full phone
      rec.Phone = (rec.Mobile !== null) ? rec.Mobile.trim() : '';
      rec.Phone += (rec.Phone.length > 0) ? ' ' : '';
      rec.Phone += (rec.PhoneRes !== null) ? rec.PhoneRes.trim() : '';
    }
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE LastName = '" + escapeSingleQuotes(compVar.formData.LastName.trim()) + "' " + 
       " AND FirstName = '" + escapeSingleQuotes(compVar.formData.FirstName.trim()) + "' " + 
       " AND Addressbook_id = " + props.addressbook_id;
  ...
  const checkFormErrors = async (formData) => {
    ...
    if (formData.FirstName.trim().length === 0 && formData.LastName.trim().length === 0) {
      return "'First Name' or 'Last Name' have to be entered";
    }
