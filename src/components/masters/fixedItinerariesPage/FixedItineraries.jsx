...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetFixedItinerariesData";
...
function FixedItineraries() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'FixedItin', keyField: 'FixedItin_id',
      masterDescField: 'Title',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Fixed Itineraries', title: 'New Fixed Itinerary',
  ...
  const fetchInitialData = async() => {
    try {
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Fixed Itineraries'});   
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo'], table: 'FixedItin', where: ' Addressbook_id = 2137 and QuotationRef is not null', x_uid: _g_users_id, x_module: 'Fixed Itineraries'});   
    } catch(err) {
      alert(err);
    }
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('SearchTags','OrderNo','(1=1)');
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'ElemCars', condition: "WHERE QuotationRef = '" + e.row.data.QuotationRef + "'", existsIn: 'Element Cars. Delete the Element details first'},
      {table: 'ElemCityGroups', condition: "WHERE QuotationRef = '" + e.row.data.QuotationRef + "'", existsIn: 'Element City Groups. Delete the Element City Group details first'},
      {table: 'ElemInterCities', condition: "WHERE QuotationRef = '" + e.row.data.QuotationRef + "'", existsIn: 'Element Inter-cities. Delete the Element Inter-cities details first'},
      {table: 'Quotations', condition: "WHERE QuotationRef = '" + e.row.data.QuotationRef + "'", existsIn: 'Tours. Delete the Tour details first'},
    ]);    
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE ((" + compVar.masterDescField + " = '" + escapeSingleQuotes(compVar.formData[compVar.masterDescField]) + "') " + 
      " OR (QuotationRef = '" + compVar.formData.QuotationRef + "')) ";
...
export default FixedItineraries;
