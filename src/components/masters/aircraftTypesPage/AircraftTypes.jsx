...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetAircraftTypeData";
...
function AircraftTypes() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'aircrafttypes', keyField: 'aircrafttypes_id',
      masterDescField: 'aircraft',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Aircraft Types', title: 'New Aircraft Type',
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['aircraft'], table: 'aircrafttypes', x_uid: _g_users_id, x_module: 'Aircraft Types'});   
    } catch(err) {
      alert(err);
    }
