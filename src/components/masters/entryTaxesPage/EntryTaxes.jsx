...
import { beforeInsert, getLookupValues, saveEditedInsertedData, checkNullErrors, convert_DbDate_To_MDY, getFieldsArray, convertToMoment } from "../../common/CommonTransactionFunctions";
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetEntryTaxData";
...
function EntryTaxes() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [], vehicleLookup: [], mainData: [],
      stateLookup: [],      
      selectedStates_id: -1, selectedState: '',  
      tableName: 'EntryTaxes', keyField: 'EntryTaxes_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Entry Taxes', title: 'New Entry Tax',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.stateLookup = await dbGetRecord({fields: ['state', 'states_id'], orders: ['state'], table: 'states', x_uid: _g_users_id, x_module: 'Entry Taxes'}); 

      compVar.vehicleLookup = await dbGetRecord({fields: ['vehicles_id', 'vehicle'], orders: ['vehicle'], table: 'vehicles', x_uid: _g_users_id, x_module: 'Entry Taxes'});    
      compVar.dbLookup[0].dataSource = compVar.vehicleLookup;
  
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Entry Taxes'});   
  ...
  const filterData = async() => {
    ...
    let whereStr = 'states_id = ' + compVar.selectedStates_id.toString();
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['wef DESC'], table: 'EntryTaxes', where: whereStr, x_uid: _g_users_id, x_module: 'Entry Taxes'});   
    } catch(err) {
      alert(err);
    }
  ...
  const addRow = async () => {
    ...
    defaultObj = {...defaultObj, 
      States_id: compVar.selectedStates_id,
    }
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE wef = '" + convert_DbDate_To_MDY(compVar.formData.Wef,1) + "' " + 
      "AND States_id = " + compVar.formData.States_id.toString() + " "; 
    ...
    // update Prev wet
    await updatePreviousWet (compVar.formData.wef, compVar.formData.Vehicles_id, compVar.formData.States_id);
  ...
  const checkFormErrors = async (formData) => {
    ...
    if ((compVar.formData.wet !== null) && convertToMoment(compVar.formData.wef) > convertToMoment(compVar.formData.wet)) {
      return 'Wef cannot be greater than Wet'      
    }
  ...
  //**********************************************************/
  const updatePreviousWet = async (wef, vehicles_id, states_id) => {

    let prevWet = convertToMoment(wef).toDate();
    prevWet.setDate(prevWet.getDate() - 1);
    prevWet = convert_DbDate_To_MDY(prevWet,1);

    const query = "UPDATE EntryTaxes " + 
      "SET wet = '" + prevWet + "' " +
      "WHERE Vehicles_id = " + vehicles_id.toString() + " " +
      "AND States_id = " + states_id.toString() + " " +
      "AND wef < '" + convert_DbDate_To_MDY(wef,1) + "' " + 
      "AND wet IS NULL";
    await dbGetRecordRaw({query: query});
  ...
  const createElementProps = () => {
    ...
    const canAdd = (compVar.selectedStates_id > 0) ? compVar.canAdd : false;
  ...
  const onStateSelect = async(e) => {
    if (e.length > 0) {
      compVar.selectedStates_id = e[0].states_id;
      compVar.selectedState = e[0].state;  
      filterData();
    }
  }
