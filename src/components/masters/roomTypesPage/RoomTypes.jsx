...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetRoomTypeData";
...
function RoomTypes() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'roomtypes', keyField: 'roomtypes_id',
      masterDescField: 'roomtype',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Room Types', title: 'New Room Type',
  ...
  const fetchInitialData = async() => {
    try{
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Room Types'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['roomtype'], table: 'roomtypes', x_uid: _g_users_id, x_module: 'Room Types'});   
    } catch(err) {
      alert(err);
    }
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'blockbookingrooms', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Block Bookings. Delete the block booking details first'},
      {table: 'bookingsaccommodation', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Booking Accommodation. Delete the booking accommodation details first'},
      {table: 'hotelrooms', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Hotel Rooms. Delete the hotel room details first'},
      {table: 'hoteltariffs', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Hotel Tariffs. Delete the hotel tariff details first'},
      {table: 'hoteltariffsindia', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Hotel Tariffs. Delete the hotel tariff details first'},
      ...
      {table: 'QuoAccommodation', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Tours Accommodation. Delete the tour accommodation details first'},
