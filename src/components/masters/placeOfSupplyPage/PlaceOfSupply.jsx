...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetPlaceOfSupplyData";
...
function PlaceOfSupply() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      stateLookup: [],
      tableName: 'PlaceOfSupply', keyField: 'PlaceOfSupply_id',
      masterDescField: 'PlaceOfSupply',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Place Of Supply', title: 'New Place Of Supply',
  ...
  const fetchInitialData = async() => {
    ...
    try {
      ...
      compVar.stateLookup = await dbGetRecord({fields: ['states_id', 'state'], orders: ['state'], table: 'states', x_uid: _g_users_id, x_module: 'Place Of Supply'});   
      compVar.dbLookup[0].dataSource = compVar.stateLookup;
  
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Place Of Supply'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['PlaceOfSupply_id'], table: 'PlaceOfSupply', x_uid: _g_users_id, x_module: 'Place Of Supply'});   
    } catch(err) {
      alert(err);
    }
  ...
  const deleteRow = async (e) => {
    ...
    error.errorMsg = 'Cannot delete records from Place of Supply';
