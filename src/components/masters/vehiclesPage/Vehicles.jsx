...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetVehicleData";
...
function Vehicles() {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      tableName: 'vehicles', keyField: 'vehicles_id',
      masterDescField: 'vehicle',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Vehicles', title: 'New Vehicle',
      ...
      admLevel: 1,
      dbLookup: [       
        {keyField: 'vehiclecategories_id', dataSource: compVar.vehicleCategoryLookup, 
        displayExpr: 'category', valueExpr: 'vehiclecategories_id', fieldList: ['category']},
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.vehicleCategoryLookup = await dbGetRecord({fields: ['vehiclecategories_id', 'category'], orders: ['category'], table: 'vehiclecategories', x_uid: _g_users_id, x_module: 'Vehicles'});   
      compVar.dbLookup[0].dataSource = compVar.vehicleCategoryLookup;
  
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Vehicles'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['vehicle'], table: 'vehicles', x_uid: _g_users_id, x_module: 'Vehicles'});   
    } catch(err) {
      alert(err);
    }
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      ...
      {table: 'carhireagents', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Car Hire Agents. Delete the car hire agent details first'},
      ...
      {table: 'costservicestransport', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Services Transport. Delete the services transport details first'},
      {table: 'ElemCars', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Elements. Delete the element details first'},
      {table: 'ElemCarsCosts', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Elements. Delete the element details first'},
      {table: 'ElemCityGroupsCosts', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Element City Groups. Delete the element city group details first'},
      {table: 'ElemExtraDayCarHireCosts', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Element Day Costs. Delete the element day cost details first'},
      {table: 'ElemInterCities', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Element Inter-Cities. Delete the element inter-city details first'},
      {table: 'ElemInterCitiesCosts', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Element Inter-Cities. Delete the element inter-city details first'},
      {table: 'ElemServices', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Element Services. Delete the element services details first'},
      {table: 'ElemServicesCosts', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Element Services. Delete the element services details first'},
      {table: 'EntryTaxes', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Entry Taxes. Delete the entry taxes details first'},
      ...
      {table: 'QuoTickets', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Tour Tickets. Delete the tour ticket details first'},
    ]);    
  ...
  //**********************************************************/
  const getSelectedVehicleCategory = (e) => {
    compVar.formData.vehiclecategories_id = e[0].vehiclecategories_id;
  }
  ...
  //**********************************************************/
  const clearVehicleCategoryLookup = () => {
    compVar.formData.vehiclecategories_id = null;
  }
  ...
  const createFormObject = () => {
    ...
    const clearVehicleCategoryLookupValues = {vehiclecategories_id: null, category: ''};
    const clearUserLookupValues = {AdmUsers_id: null, uid: ''};

    const initialVehicleCategoryLookupValues = getLookupValues(
      clearVehicleCategoryLookupValues,compVar.vehicleCategoryLookup, 
      ['vehiclecategories_id','category'], compVar.formData.vehiclecategories_id);
    ...
    return {...defaultFormObject,
      ...
      formHelp: formHelp,
      clearLookup: [clearVehicleCategoryLookup, clearUserLookup],
      getSelectedRecord: [getSelectedVehicleCategory, getSelectedUser],
      initialLookupValues: [initialVehicleCategoryLookupValues, initialUserLookupValues],
      clearLookupValues: [clearVehicleCategoryLookupValues, clearUserLookupValues],
    }
