...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetExchRateData";
...
function ExchRates() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      currencyLookup: [],
      selectedCurrencies_id: -1, selectedCurrency: '',  
      tableName: 'currencydetails', keyField: 'currencydetails_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Exchange Rates', title: 'New Exchange Rate',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Exch Rates'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['wef DESC'], table: 'currencydetails', where: whereStr, x_uid: _g_users_id, x_module: 'Exch Rates'});   
    } catch(err) {
      alert(err);
    }
