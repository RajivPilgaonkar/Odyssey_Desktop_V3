...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetExclusionDetailsData";
...
function ExclusionDetails(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'QuoExclusionDetails', keyField: 'QuoExclusionDetails_id',
      masterDescField: 'QuoExclusionDetail',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Exclusion Details', title: 'New Exclusion Detail',
  ...
  const fetchInitialData = async() => {
    try {
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Exclusions'});   
  ...
  const filterData = async() => {
    ...
    let whereStr = 'QuoExclusions_id = ' + props.exclusionsId.toString();
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['SrNo'], table: 'QuoExclusionDetails', where: whereStr, x_uid: _g_users_id, x_module: 'Exclusions'});   
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('QuoExclusionDetails','SrNo','QuoExclusions_id = ' + props.exclusionsId.toString());
    const nextSrNo = (nextObj.length > 0) ? nextObj[0].OrderNo+1 : 1;

    // add other code like next sr no, yearref ...
    defaultObj = {...defaultObj, 
      SrNo: nextSrNo,
      QuoExclusions_id: props.exclusionsId
    }
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'QuoExclusionDetailsDisplay', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Presto Exclusions Display'},
      {table: 'QuoExclDetails', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Exclusion Details'},
    ]);    
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE CAST(" + compVar.masterDescField + " AS VARCHAR(2000)) = '" + escapeSingleQuotes(compVar.formData[compVar.masterDescField]) + "' " + 
      "AND QuoExclusions_id = " + props.exclusionsId.toString();
...
export default ExclusionDetails;
