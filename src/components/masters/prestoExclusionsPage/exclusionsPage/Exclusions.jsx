...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetExclusionsData";
...
function Exclusions(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'QuoExclusions', keyField: 'QuoExclusions_id',
      masterDescField: 'QuoExclusion',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Exclusions', title: 'New Exclusion',
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['SrNo'], table: 'QuoExclusions', x_uid: _g_users_id, x_module: 'Exclusions'});       
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('QuoExclusions','SrNo','(1=1)');
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'QuoExcl', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Presto Exclusions. Delete the Presto Exclusion Details first'},
      {table: 'QuoExclusionDetails', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Exclusion Details. Delete the Exclusion Details first'},
    ]);    
  ...
  const returnId = async (e) => {
      ...
      if (props.onExclusionChange !== undefined) {
        await props.onExclusionChange({
          exclusions_id: compVar.focusedRowKey,
        });
      };  
  }
  ...
  const onFocusedRowChanged = (e) => {
    ...
    if (e.row !== undefined) {
      ...
      if (compVar.mainData.length > 0) {
        ...
        //forceRender();        
        // pass back the active exclusions_id
        returnId()
      }
