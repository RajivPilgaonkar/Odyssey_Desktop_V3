...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetInvExchRateData";
...
function InvExchRate() {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      tableName: 'InvExchRates', keyField: 'InvExchRates_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Invoice Exchange Rates', title: 'New Invoice Exchange Rate',
  ...
  const fetchInitialData = async() => {
    ...
    try {
      ...
      compVar.currencyLookup = await dbGetRecord({fields: ['currencies_id', 'currencycode'], orders: ['currencycode'], table: 'currencies', where: whereStr, x_uid: _g_users_id, x_module: 'Invoice Exch Rate'}); 

      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Invoice Exch Rate'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['wef DESC'], table: 'InvExchRates', where: whereStr, x_uid: _g_users_id, x_module: 'Invoice Exch Rate'});   
    } catch(err) {
      alert(err);
    }
  ...
  const addRow = async () => {
    ...
    defaultObj = {...defaultObj, 
      Currencies_id: compVar.selectedCurrencies_id,
    }
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE wef = '" + convert_DbDate_To_MDY(compVar.formData.wef,1) + "' " + 
  ...
  const createElementProps = () => {
    ...
    const canAdd = (compVar.selectedCurrencies_id > 0) ? compVar.canAdd : false;
