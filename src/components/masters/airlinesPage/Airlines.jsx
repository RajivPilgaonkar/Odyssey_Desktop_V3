...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetAirlinesData";
...
function Airlines() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'airlines', keyField: 'airlines_id',
      masterDescField: 'airline',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Airlines', title: 'New Airline',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Airlines'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['airline'], table: 'airlines', x_uid: _g_users_id, x_module: 'Airlines'});   
    } catch(err) {
      alert(err);
    }
