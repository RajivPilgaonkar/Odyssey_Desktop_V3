...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetNetworkData";
...
function Networks(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'PrincipalAgentNetworks', keyField: 'PrincipalAgentNetworks_id',
      masterDescField: 'PrincipalAgentNetwork',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Principal Agent Networks', title: 'New Principal Agent Network',
  ...
  const fetchInitialData = async() => {
    ...
    try {
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Principal Agent Networks'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['PrincipalAgentNetworks_id'], table: 'PrincipalAgentNetworks', x_uid: _g_users_id, x_module: 'Principal Agent Networks'});       
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'PrincipalAgentNetworkDetails', condition: 'WHERE PricipalAgentNetworks_id = ' + e.row.data[compVar.keyField], existsIn: 'Principal Agents'},
    ]);    
  ...
  const returnId = async (e) => {
      ...
      if (props.onNetworkChange !== undefined) {
        await props.onNetworkChange({
          networks_id: compVar.focusedRowKey,
        });
      };  
  }
  ...
  const createElementProps = () => {
    ...
    return {
      numButtons: 1,
      buttonListObj: [
        {visible: true, options: {icon: "add", onClick: addRow, hint: 'Add a new record'}},
      ],
