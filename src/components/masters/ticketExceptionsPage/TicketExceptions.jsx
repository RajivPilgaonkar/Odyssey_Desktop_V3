...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetTicketExceptionData";
...
function TicketExceptions() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'TicketExceptions', keyField: 'TicketExceptions_id',
      masterDescField: 'TrainNo',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Train Exceptions', title: 'New Train Exception',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Ticket Exceptions'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['TrainNo'], table: 'TicketExceptions', x_uid: _g_users_id, x_module: 'Ticket Exceptions'});   
    } catch(err) {
      alert(err);
    }
...
export default TicketExceptions;
