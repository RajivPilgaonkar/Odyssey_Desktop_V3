...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetMarginData";
...
function Margins() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [], typeLookup: [], mainData: [],
      clonedMainData: [],
      selectedTypes_id: -1, selectedType: '',  
      tableName: 'FitMargins', keyField: 'FitMargins_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Margins', title: 'New Margin',
      ...
      elementTypesData:
        [
          {id: 1, type: 1, text: 'Tickets'},
          {id: 2, type: 2, text: 'Accommodation'},
          {id: 3, type: 3, text: 'Sightseeing'},
          {id: 4, type: 4, text: 'Transfers'},
          {id: 5, type: 5, text: 'Transport'},
          {id: 6, type: 10, text: 'Packages'},
        ]
  ...
  const fetchInitialData = async() => {
    ...
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Margins'});   
  ...
  const filterData = async() => {
    ...
    let whereStr = 'TrsType = ' + compVar.selectedTypes_id.toString();
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo'], table: 'FitMargins', where: whereStr, x_uid: _g_users_id, x_module: 'Margins'});   
      compVar.clonedMainData =  [...compVar.mainData];     
    } catch(err) {
      alert(err);
    }

    changeDisplayArrangement();
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('FitMargins','OrderNo','TrsType = ' + compVar.selectedTypes_id.toString());
    const nextSrNo = (nextObj.length > 0) ? nextObj[0].OrderNo+1 : 1;

    // add other code like next sr no, yearref ...
    defaultObj = {...defaultObj, 
      TrsType: compVar.selectedTypes_id,
      OrderNo: nextSrNo
    }
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE TrsTypeDesc = '" + escapeSingleQuotes(compVar.formData.TrsTypeDesc) + "' " + 
      "AND TrsType = " + compVar.formData.TrsType.toString() + " "; 
  ...
  const createElementProps = () => {
    ...
    const canAdd = (compVar.selectedTypes_id > 0) ? compVar.canAdd : false;
  ...
  const createStateParams = () => {
    ...
    return (
      <DropDownGrid
        listArray={compVar.elementTypesData}
        fieldList={['text']}
        valueExpr="type"
        displayExpr="text"
        label="Type"
  ...
  //*********************************************************/
  const onTypeSelect = async(e) => {
    if (e.length > 0) {
      compVar.selectedTypes_id = e[0].type;
      compVar.selectedType = e[0].text;  
      await filterData();
    }
  ...
  //*********************************************************/
  const changeDisplayArrangement = () => {

    tableHeaderArray.forEach(rec => {
      if (rec.field === 'FromAmt') {
        rec.visible = (compVar.selectedTypes_id === 2) ? true : false;
        rec.visibleInForm = (compVar.selectedTypes_id === 2) ? true : false;
      }
      if (rec.field === 'ToAmt') {
        rec.visible = (compVar.selectedTypes_id === 2) ? true : false;
        rec.visibleInForm = (compVar.selectedTypes_id === 2) ? true : false;
