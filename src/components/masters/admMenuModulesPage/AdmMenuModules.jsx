...
import { beforeInsert, getLookupValues, saveEditedInsertedData, checkNullErrors, getFieldsArray, escapeSingleQuotes, convert_DbDate_To_MDY } from "../../common/CommonTransactionFunctions";
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetAdmMenuModulesData";
...
function AdmMenuModules() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      moduleTypeLookup:[],
      tableName: 'AdmMenuModules', keyField: 'AdmMenuModules_id',
      masterDescField: 'AdmMenuModule',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Menu Modules', title: 'New Menu Module',
      ...
      admLevel: 1,
      dbLookup: [      
        {keyField: 'AdmModuleTypes_id', dataSource: compVar.moduleTypeLookup, 
        displayExpr: 'AdmModuleType', valueExpr: 'AdmModuleTypes_id', fieldList: ['AdmModuleType']},
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.moduleTypeLookup = await dbGetRecord({fields: ['AdmModuleTypes_id', 'AdmModuleType'], orders: ['AdmModuleTypes_id'], table: 'AdmModuleTypes'});   
      compVar.dbLookup[0].dataSource = compVar.moduleTypeLookup;  
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['AdmMenuModuleNo'], table: 'AdmMenuModules'});   
    } catch(err) {
      alert(err);
    }
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE ((" + compVar.masterDescField + " = '" + escapeSingleQuotes(compVar.formData[compVar.masterDescField]) + "')  OR " + 
      "(AdmMenuModuleNo = " + compVar.formData.AdmMenuModuleNo + ")) ";
  ...
  //**********************************************************/
  const getSelectedModuleType = (e) => {
    compVar.formData.AdmModuleTypes_id = e[0].AdmModuleTypes_id;
  }
  ...
  //**********************************************************/
  const clearModuleTypeLookup = () => {
    compVar.formData.AdmModuleTypes_id = null;
  }
  ...
  const createFormObject = () => {
    ...
    const clearModuleTypeLookupValues = {AdmModuleTypes_id: null, AdmModuleType: ''};
    const clearUserLookupValues = {AdmUsers_id: null, uid: ''};

    const initialModuleTypeLookupValues = getLookupValues(
      clearModuleTypeLookupValues,compVar.moduleTypeLookup, 
      ['AdmModuleTypes_id','AdmModuleType'], compVar.formData.AdmModuleTypes_id);
    ...
    return {...defaultFormObject,
      ...
      formHelp: formHelp,
      clearLookup: [clearModuleTypeLookup, clearUserLookup],
      getSelectedRecord: [getSelectedModuleType, getSelectedUser],
      initialLookupValues: [initialModuleTypeLookupValues, initialUserLookupValues],
      clearLookupValues: [clearModuleTypeLookupValues, clearUserLookupValues],
    }
