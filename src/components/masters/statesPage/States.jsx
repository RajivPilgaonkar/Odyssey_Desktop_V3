...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetStateData";
...
function AircraftTypes() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'states', keyField: 'states_id',
      masterDescField: '[state]',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'States', title: 'New State',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'States'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['[state]'], table: 'states', x_uid: _g_users_id, x_module: 'States'});   
    } catch(err) {
      alert(err);
    }
