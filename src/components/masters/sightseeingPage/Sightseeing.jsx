...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetSightseeingData";
...
import {getDaysOfOperation, getSightseeingTimings}  from "../../common/GetDescFromIds";
...
function Sightseeing() {
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.cityLookup = await dbGetRecord({fields: ['cities_id', 'city'], orders: ['city'], table: 'cities', where: whereStr, x_uid: _g_users_id, x_module: 'Sightseeing'});   
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Sightseeing'});   
  ...
  const filterData = async() => {
    ...
    try {
      const whereStr = "Cities_id = " + compVar.selectedCities_id.toString() + " " + 
        activeStr + " " + 
        "AND Transfer = 0";
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['defaultorder, description'], table: '[services]', where: whereStr, x_uid: _g_users_id, x_module: 'Sightseeing'});   

      // Update days of operation as a string (stored as a bit)
      for (const rec of compVar.mainData) {
        const dayObj = await getDaysOfOperation(rec.DaysOfOperation); 
        rec.DayString = dayObj.daysOfOperation;
      }

      // Update timings for sightseeing
      for (const rec of compVar.mainData) {
        const timing = await getSightseeingTimings(rec.Services_id); 
        rec.Timing = timing;
      }
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('Services','DefaultOrder','Transfer = 0 AND cities_id = ' + compVar.selectedCities_id.toString());
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      ...
      {table: 'ServiceTimings', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Service Timings. Delete the service timing details first'},
  ...
  //**********************************************************/
  const getSelectedHotel = (e) => {
    compVar.formData.NotHotelAddressbook_id = e[0].Addressbook_id;
  }
  ...
  const clearHotelLookup = (e) => {
    compVar.formData.NotHotelAddressbook_id = null;
  }
  ...
  const onSelectedDaysOfOperation = (e) => {
    compVar.formData.DaysOfOperation = e.dayBit;
  ...
  const createFormObject = () => {
    ...
    const initialHotelLookupValues = getLookupValues(
      clearHotelLookupValues,compVar.agentLookup, 
      ['Addressbook_id','OrgCity'], compVar.formData.NotHotelAddressbook_id);
    ...
    return {...defaultFormObject,
      ...
      formHelp: formHelp,
      clearLookup: [clearAgentLookup, clearHotelLookup, clearUserLookup],
      getSelectedRecord: [getSelectedAgent, getSelectedHotel, getSelectedUser],
      initialLookupValues: [initialAgentLookupValues, initialHotelLookupValues, initialUserLookupValues],
      clearLookupValues: [clearAgentLookupValues, clearHotelLookupValues, clearUserLookupValues],
      ...
      <>
        <div className="master-grid-container" style={{height: containerHeight, flexDirection: 'column', justifyContent: 'flex-start'}}>
          ...
          {daysOperationVisible &&
            <DaysOfOperation
              daysOfOperation={compVar.formData.DaysOfOperation}
