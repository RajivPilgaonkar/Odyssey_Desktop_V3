...
import { getLookupValues, saveEditedInsertedData, checkNullErrors, convert_DbDate_To_MDY, getFieldsArray } from "../../common/CommonTransactionFunctions";
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetInvPymtBeneficiaryData";
...
function InvPymtBeneficiary() {
  ...
  const [activeCurrency, setActiveCurrency] = useState(true);
  ...
  let _g_currencies_id = useSelector(state => state.params.currencies_id) || -1;				
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      currencyLookup: [],
      selectedCurrencies_id: _g_currencies_id, selectedCurrency: '',  
      tableName: 'Currencies', keyField: 'Currencies_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Invoice Payment Beneficiary Details', title: '',
      errorMsg: '', focusedRowKey: -1,
      tabs: [{title: 'Beneficiary Details', index: 0},{title: 'Bank Instructions', index: 1}],
  ...
  const fetchInitialData = async() => {
    ...
    try {
      ...
      const whereStr = activeCurrency ? 'Currencies_id IN ' + 
        '(SELECT Currencies_id FROM InvExchRates WHERE DATEDIFF(year, wef, GETDATE()) <= 3) ' : '(1=1)';
      compVar.currencyLookup = await dbGetRecord({fields: ['currencies_id', 'currencycode'], orders: ['currencycode'], table: 'currencies', where: whereStr, x_uid: _g_users_id, x_module: 'Invoice Pymt Beneficiary'}); 

      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Invoice Pymt Beneficiary'});   
      compVar.dbLookup[0].dataSource = compVar.userLookup;  
    } catch(err) {
      alert(err);
    }

    // If currencies_id picked from redux, set currency as well for display in drop down
    if (compVar.selectedCurrencies_id > -1) {
      const idx = compVar.currencyLookup.findIndex(rec => rec.currencies_id === compVar.selectedCurrencies_id);
      if (idx > -1) {
        compVar.selectedCurrency = compVar.currencyLookup[idx].currencycode;
      }
    }
  ...
  const filterData = async() => {
    ...
    let whereStr = 'currencies_id = ' + compVar.selectedCurrencies_id.toString();
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['currencycode'], table: 'Currencies', where: whereStr, x_uid: _g_users_id, x_module: 'Invoice Pymt Beneficiary'});   
    } catch(err) {
      alert(err);
    }
  ...
  //**********************************************************/
  const createCurrencyParams = (mode) => {
      ...
      return (
        <>
          <div className="master-grid-params-switch-container"> 
            ...
            <div style={{height: 20}}>
              <Switch height={20} width={40} onChange={onActiveSwitchChange} checked={activeCurrency} uncheckedIcon={false}/>
            </div>
          </div>
        </>
    ...
    return (
      <DropDownGrid
        listArray={compVar.currencyLookup}
  ...
  //*********************************************************/
  const onCurrencySelect = async(e) => {
    if (e.length > 0) {
      compVar.selectedCurrencies_id = e[0].currencies_id;
      compVar.selectedCurrency = e[0].currencycode;  

      // save trainNo to the redux store
      dispatch(setParamValues({currencies_id: compVar.selectedCurrencies_id}));
...
export default InvPymtBeneficiary;
