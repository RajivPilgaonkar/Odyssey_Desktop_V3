...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetSearchTagData";
...
function SearchTags() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'SearchTags', keyField: 'SearchTags_id',
      masterDescField: 'SearchTag',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Search Tags', title: 'New Search Tag',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Search Tags'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo'], table: 'SearchTags', x_uid: _g_users_id, x_module: 'Search Tags'});   
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'AddressbookSearchTags', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Addressbook Search Tags. Delete the Addressbook Search Tags first'},
    ]);    
