...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetTaxData";
...
function Taxes() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'taxes', keyField: 'taxes_id',
      masterDescField: 'tax',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Taxes', title: 'New Tax',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Taxes'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['tax'], table: 'taxes', x_uid: _g_users_id, x_module: 'Taxes'});   
    } catch(err) {
      alert(err);
    }
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'centraltaxes', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Central Taxes. Delete the central tax details first'},
