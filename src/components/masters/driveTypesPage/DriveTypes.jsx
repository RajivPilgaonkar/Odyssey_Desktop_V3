...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetDriveTypesData";
...
function DriveTypes() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'DriveTypes', keyField: 'DriveTypes_id',
      masterDescField: 'DriveType',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Drive Types', title: 'New Drive Type',
      errorMsg: '', focusedRowKey: -1,
      tabs: [{title: 'Main', index: 0},{title: 'Additional', index: 1}],
      canAdd: false, canModify: false, canDeleteRow: false,
  ...
  const fetchInitialData = async() => {
    try {
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Drive Types'});   
    } catch(err) {
      alert(err);
    }
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['DriveTypes_id'], table: 'DriveTypes', x_uid: _g_users_id, x_module: 'Drive Types'});   
    } catch(err) {
      alert(err);
    }
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'QuoLines', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Tours. Delete the tour details first'},
      ...
      {table: 'VouchersTransport', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Vouchers transport. Delete the voucher transport details first'},
      {table: 'VouchersTransport', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Vouchers transport. Delete the voucher transport details first'},
    ]);    
