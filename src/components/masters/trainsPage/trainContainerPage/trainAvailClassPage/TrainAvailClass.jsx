...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetTrainAvailClassData";
...
function TrainAvailClass(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      classLookup: [],
      tableName: 'TrainAvailableClass', keyField: 'TrainAvailableClass_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Available Classes', title: 'New Class',
  ...
  const fetchInitialData = async() => {
    try {
      const whereStr = 'tickets_id = 2';
      compVar.classLookup = await dbGetRecord({fields: ["class_id", "[class] + ' ' + COALESCE(code,'') AS classCode", "tickets_id"], orders: ['[class]'], table: 'class', where: whereStr, x_uid: _g_users_id, x_module: 'Trains'});   
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['class_id'], table: 'TrainAvailableClass', where: whereStr, x_uid: _g_users_id, x_module: 'Trains'});   
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE Class_id = " + compVar.formData.Class_id.toString() + " AND " +
...
export default TrainAvailClass;
