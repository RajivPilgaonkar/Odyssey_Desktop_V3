...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetConsultantData";
...
function Consultants() {
  ...
  const [activeConsultant, setActiveConsultant] = useState(true);
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      tableName: 'Consultants', keyField: 'Consultants_id',
      masterDescField: 'Consultant',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Consultants', title: 'New Consultant',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Consultants'});   
  ...
  const filterData = async() => {
    ...
    const active = activeConsultant ? 1 : 0;
    const activeStr = (activeConsultant) ? " AND Active = " + active.toString() : "";

    let fieldArray = getFieldsArray(tableHeaderArray);
    let whereStr = 'Addressbook_id = ' + compVar.selectedAgents_id.toString() + activeStr;
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['Consultant'], table: 'Consultants', where: whereStr, x_uid: _g_users_id, x_module: 'Consultants'});   
    } catch(err) {
      alert(err);
    }
  ...
  const createStateParams = () => {
    ...
    return (
      <>
        ...
        />
        <Switch height={20} width={40} onChange={onActiveSwitchChange} checked={activeConsultant} uncheckedIcon={false}/>
      </>
