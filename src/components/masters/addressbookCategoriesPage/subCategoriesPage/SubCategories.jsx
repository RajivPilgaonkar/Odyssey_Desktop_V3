...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetSubCategoryData";
...
function SubCategories(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'ContactSubCategories', keyField: 'ContactSubCategories_id',
      masterDescField: 'ContactSubCategory',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Sub-Categories', title: 'New Sub-Category',
      errorMsg: '', focusedRowKey: -1,
      tabs: [{title: 'Main', index: 0},{title: 'Additional', index: 1}],
      canAdd: true, canModify: true, canDeleteRow: true,
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Addressbook Categories'});   
  ...
  const filterData = async() => {
    ...
    let whereStr = 'ContactCategories_id = ' + props.contactCategories_id.toString();
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo'], table: 'ContactSubCategories', where: whereStr, x_uid: _g_users_id, x_module: 'Addressbook Categories'});   
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('ContactSubCategories','OrderNo','ContactCategories_id = ' + props.contactCategories_id.toString());
    const nextSrNo = (nextObj.length > 0) ? nextObj[0].OrderNo+1 : 1;

    // add other code like next sr no, yearref ...
    defaultObj = {...defaultObj, 
      OrderNo: nextSrNo,
      ContactCategories_id: props.contactCategories_id
    }
  ...
  const deleteRow = async (e) => {
    ...
    // In this case the delete can be triggered from the button or from the grid -- edit/delete
    // ... Sometimes, the grid does not scroll horizontally and the delete button cannot be seen
    let keyField = '';
    if (e.row !== undefined && e.row.data !== undefined) {
      keyField = e.row.data[compVar.keyField]      
    } else {
      keyField = compVar.focusedRowKey;
    }

    const error = await canDelete([
      {table: 'ContactCategoryServices', condition: 'WHERE ' + compVar.keyField + ' = ' + keyField.toString(), existsIn: 'Addressbook Services.'},
      {table: 'AddressbookSubcategories', condition: 'WHERE ' + compVar.keyField + ' = ' + keyField.toString(), existsIn: 'Addressbook Services.'},
      {table: 'AddressbookServices', condition: 'WHERE ' + compVar.keyField + ' = ' + keyField.toString(), existsIn: 'Addressbook Services.'},
    ]);    
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE " + compVar.masterDescField + " = '" + escapeSingleQuotes(compVar.formData[compVar.masterDescField]) + "' " + 
      "AND ContactCategories_id = " + props.contactCategories_id.toString();
  ...
  const returnId = async (e) => {
    ...
    if (props.onSubCategoryChange !== undefined) {
      await props.onSubCategoryChange({
        contactSubCategories_id: compVar.focusedRowKey,
        contactSubCategory: ''
      });        
    };
  ...
  const onFocusedRowChanged = (e) => {
    ...
    if (e.row !== undefined) {
      ...
      if (compVar.mainData.length > 0) {
        ...
        //forceRender();        
        // pass back the active categories_id
        returnId()
      }
  ...
  const createElementProps = () => {
    ...
    return {
      ...
      buttonListObj: [
        ...
        {visible: canAdd, options: {icon: "remove", onClick: deleteRow, hint: 'Delete this row'}},  
      ],
