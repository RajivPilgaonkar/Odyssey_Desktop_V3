...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetCategoryData";
...
function Categories(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'ContactCategories', keyField: 'ContactCategories_id',
      masterDescField: 'ContactCategory',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Categories', title: 'New Category',
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo'], table: 'ContactCategories', x_uid: _g_users_id, x_module: 'Addressbook Categories'});   
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('ContactCategories','OrderNo','(1=1)');
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'ContactSubCategories', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Sub-Categories Tags. Delete the Sub-Category Tags first'},
    ]);    
  ...
  const returnId = async (e) => {
      ...
      if (props.onCategoryChange !== undefined) {
        await props.onCategoryChange({
          contactCategories_id: compVar.focusedRowKey,
          contactCategory: ''
        });
      };  
  }
