...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetAddressbookServiceData";
...
function AddressbookServices(props) {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'AddressbookServices', keyField: 'AddressbookServices_id',
      masterDescField: 'AddressbookService',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Services Offered', title: 'New Service',
  ...
  const filterData = async() => {
    ...
    let whereStr = 'ContactSubCategories_id = ' + props.contactSubCategories_id.toString();
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['OrderNo'], table: 'AddressbookServices', where: whereStr, x_uid: _g_users_id, x_module: 'Addressbook Categories'});   
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('AddressbookServices','OrderNo','ContactSubCategories_id = ' + props.contactSubCategories_id.toString());
    const nextSrNo = (nextObj.length > 0) ? nextObj[0].OrderNo+1 : 1;

    // add other code like next sr no, yearref ...
    defaultObj = {...defaultObj, 
      OrderNo: nextSrNo,
      ContactSubCategories_id: props.contactSubCategories_id
    }
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      {table: 'AddressbookCategoryServices', condition: 'WHERE ' + compVar.keyField + ' = ' + keyField.toString(), existsIn: 'Addressbook. Delete the Addressbook details first'},
    ]);    
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE " + compVar.masterDescField + " = '" + escapeSingleQuotes(compVar.formData[compVar.masterDescField]) + "' " + 
      "AND ContactSubCategories_id = " + props.contactSubCategories_id.toString();
