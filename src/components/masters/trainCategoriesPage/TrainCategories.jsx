...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetTrainCategoryData";
...
function TrainCategories() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      clonedMainData: [],
      tableName: 'TrainCategories', keyField: 'TrainCategories_id',
      masterDescField: 'Category',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Train Categories', title: 'New Train Category',
  ...
  const fetchInitialData = async() => {
    try{
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Train Categories'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['DefaultOrder'], table: 'TrainCategories', x_uid: _g_users_id, x_module: 'Train Categories'});   
  ...
  const addRow = async () => {
    ...
    const nextObj = await getNextSrNo('TrainCategories','DefaultOrder','(1=1)');
  ...
  const deleteRow = async (e) => {
    ...
    const error = await canDelete([
      ...
      {table: 'TrainFaresIndia', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Train Fares. Delete the Train Fares details first'},
      {table: 'trainsectorfaresindia', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Train Fares. Delete the Train Fares details first'},
      {table: 'trains', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Trains. Delete the Train details first'},
      {table: 'TrainExtraCharges', condition: 'WHERE ' + compVar.keyField + ' = ' + e.row.data[compVar.keyField], existsIn: 'Train Extra Charges. Delete the Train extra charge details first'},
    ]);    
  ...
  const onFocusedRowChanged = (e) => {
    ...
    if (e.row !== undefined) {
      ...
      compVar[compVar.keyField] = e.row.data[compVar.keyField];

      if (compVar.mainData.length > 0) {
        compVar.focusedRowKey = compVar[compVar.keyField];
        forceRender();
      }
  ...
  const createElementProps = () => {
    ...
    return {
      ...
      buttonListObj: [
        ...
        {visible: !rowDragging, options: {icon: "orderedlist", onClick: rowDraggingToggle, hint: 'Reorder using drag & drop'}},
