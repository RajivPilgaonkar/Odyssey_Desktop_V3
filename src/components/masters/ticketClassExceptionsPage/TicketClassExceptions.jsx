...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetTicketClassExceptionData";
...
function TicketClassExceptions() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      classLookup: [],
      tableName: 'TicketClassExceptions', keyField: 'TicketClassExceptions_id',
      masterDescField: 'TrainNo',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Train Class Exceptions', title: 'New Train Class Exception',
      ...
      dbLookup: [       
        ...
        displayExpr: 'class', valueExpr: 'class_id', fieldList: ['class']},
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.classLookup = await dbGetRecord({fields: ['class_id', 'class'], orders: ['class'], table: 'class', where: 'tickets_id = 2', x_uid: _g_users_id, x_module: 'Ticket Class Exceptions'});   
      compVar.dbLookup[0].dataSource = compVar.classLookup;
  
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Ticket Class Exceptions'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['TrainNo'], table: 'TicketClassExceptions', x_uid: _g_users_id, x_module: 'Ticket Class Exceptions'});   
    } catch(err) {
      alert(err);
    }
  ...
  const editRow = async (e) => {
    ...
    afterEdit(compVar, e);
    compVar.formTitle += ' ' + compVar.formData.TrainName;
    toggleEditPopup();    
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE " + compVar.masterDescField + " = '" + compVar.formData[compVar.masterDescField] + "' ";
  ...
  const createFormObject = () => {
    ...
    const clearClassLookupValues = {class_id: null, class: ''};
    const clearUserLookupValues = {AdmUsers_id: null, uid: ''};

    const initialClassLookupValues = getLookupValues (
      clearClassLookupValues, compVar.classLookup, 
      ['class_id','class'], compVar.formData.Class_id);
    ...
    return {...defaultFormObject,
      ...
      formHelp: formHelp,
      clearLookup: [clearClassLookup, clearUserLookup],
      getSelectedRecord: [getSelectedClass, getSelectedUser],
      initialLookupValues: [initialClassLookupValues, initialUserLookupValues],
      clearLookupValues: [clearClassLookupValues, clearUserLookupValues],
    }
...
export default TicketClassExceptions;
