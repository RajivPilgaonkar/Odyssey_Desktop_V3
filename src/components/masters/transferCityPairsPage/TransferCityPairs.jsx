...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetTransferCityPairData";
...
function TransferCityPairs() {
  ...
  useEffect (() => {
    ...
    compVar = {
      ...
      tableName: 'TransferCityPairs', keyField: 'TransferCityPairs_id',
      masterDescField: '',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Transfer City Pairs', title: 'New Transfer City Pair',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.cityLookup = await dbGetRecord({fields: ['cities_id', 'city'], orders: ['city'], table: 'cities', where: whereStr, x_uid: _g_users_id, x_module: 'Transfer City Pairs'});   
      compVar.dbLookup[0].dataSource = compVar.cityLookup;
      compVar.dbLookup[1].dataSource = compVar.cityLookup;

      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Transfer City Pairs'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['TransferCityPairs_id'], table: 'TransferCityPairs', x_uid: _g_users_id, x_module: 'Transfer City Pairs'});   
    } catch(err) {
      alert(err);
    }
  ...
  const saveFormData = async () => {
    ...
    let condition = "WHERE FromCities_id = " + compVar.formData.FromCities_id.toString() + " " + 
...
export default TransferCityPairs;
