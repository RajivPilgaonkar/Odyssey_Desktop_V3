...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetMealPlanData";
...
function MealPlans() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'mealplans', keyField: 'mealplans_id',
      masterDescField: 'mealplan',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Meal Plans', title: 'New Meal Plan',
  ...
  const fetchInitialData = async() => {
    try {
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Meal Plans'});   
  ...
  const filterData = async() => {
    ...
    try {
      ...
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['mealplan'], table: 'mealplans', x_uid: _g_users_id, x_module: 'Meal Plans'});   
    } catch(err) {
      alert(err);
    }
