...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetTrainDeadlineData";
...
import {getTrainName} from "../../common/GetDescFromIds";
...
function TrainDeadlines() {
  ...
  useEffect (() => {
    ...
    compVar = {
      userLookup: [],  mainData: [],
      tableName: 'TicketDeadlines', keyField: 'TicketDeadlines_id',
      masterDescField: 'TrainNo',
      formData: [], formOldData: [], formMode: -1, formTitle: 'ABC',
      mainTitle: 'Ticket Deadlines', title: 'New Deadline',
  ...
  const fetchInitialData = async() => {
    try {
      ...
      compVar.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', x_uid: _g_users_id, x_module: 'Train Deadlines'});   
  ...
  const filterData = async() => {
    ...
    try {
      compVar.mainData =  await dbGetRecord({fields: fieldArray, orders: ['TrainNo'], table: 'TicketDeadlines', x_uid: _g_users_id, x_module: 'Train Deadlines'});
    } catch(err) {
      alert(err);
    }
